# Cpp2IL.VerifyCheck - Tier 1 differential fuzzing
...
1. it is `static` and has a CIL body;
2. every parameter type and the return type is a primitive (`bool char sbyte byte short ushort int uint
   ...
   arrays, no generics, and `void` returns are excluded because a signature over one would hash the
...
5. every `call`/`callvirt`/`newobj` target resolves and is itself in the whitelist. The whitelist is a
   *greatest* fixed point over all body-safe methods, which is what lets mutually recursive maths
   helpers in. Instance methods on primitive-only structs are whitelist members (their `this` is a
   managed pointer to a struct the harness owns) but are never invoked directly.
...
hosts' class constructors having produced the same tables.
...
```
0x10, <every input leaf: kind byte + 64-bit raw value>,
      0x00, 0x11, <every output leaf: kind byte + raw value>      (returned)
...
2. **Read the Phase 1 file for the method list.** Each entry's `key` is
   `Namespace.Type::Method(ParamType,...)->ReturnType`, built by `MethodKeys` from names and shapes only:
   ...
   Skip entries with `supported: false`, `nonDeterministic: true` or `abortedAfter > 0`.
...
3. **Resolve each key to the real native method.** Il2CppInterop's generated assemblies carry the
   ...
   * *Faithful:* call the native thunk directly through
     `MethodFuzzer.Run(key, parameterTypes, returnType, invoke, plan)`. Every selected signature is
     ...
     pointer from step 3 and must be passed or the callee reads garbage. Build one
