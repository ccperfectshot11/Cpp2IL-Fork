...
namespace NetSpy.AsmEditor.Assembly {
	[ExportAutoLoaded]
	...
	sealed class RemoveAssemblyCommand : IUndoCommand {
		...
		static string GetHeader(TreeNodeData[] nodes) {
			if (nodes.Length == 1)
				return string.Format(NetSpy_AsmEditor_Resources.RemoveCommand, UIUtilities.EscapeMenuItemHeader(nodes[0].ToString()!));
			return string.Format(NetSpy_AsmEditor_Resources.RemoveAssembliesCommand, nodes.Length);
		}
		...
		public string Description => NetSpy_AsmEditor_Resources.RemoveAssemblyCommand;
	...
	sealed class AssemblySettingsCommand : IUndoCommand {
		...
		public string Description => NetSpy_AsmEditor_Resources.EditAssemblyCommand2;
	...
	sealed class CreateAssemblyCommand : IUndoCommand {
		...
		static void Execute(Lazy<IUndoCommandService> undoCommandService, IAppService appService, DocumentTreeNodeData[] nodes) {
			...
			var win = new AssemblyOptionsDlg();
			win.Title = NetSpy_AsmEditor_Resources.CreateAssemblyCommand2;
			win.DataContext = data;
			win.Owner = appService.MainWindow;
			if (win.ShowDialog() != true)
				return;
		...
		public string Description => NetSpy_AsmEditor_Resources.CreateAssemblyCommand2;
