	...
	sealed class EditCodeVMCreator {
		...
		public string? GetHeader(CompilationKind kind) {
			...
			if (info is null)
				return null;
			switch (kind) {
			case CompilationKind.EditAssembly:	return string.Format(NetSpy_AsmEditor_Resources.EditAssemblyCode2, info.Value.decompiler.GenericNameUI);
			case CompilationKind.EditMethod:	return string.Format(NetSpy_AsmEditor_Resources.EditMethodBodyCode, info.Value.decompiler.GenericNameUI);
			case CompilationKind.AddClass:		return string.Format(NetSpy_AsmEditor_Resources.EditCodeAddClass2, info.Value.decompiler.GenericNameUI);
			case CompilationKind.EditClass:		return string.Format(NetSpy_AsmEditor_Resources.EditCodeEditClass2, info.Value.decompiler.GenericNameUI);
			case CompilationKind.AddMembers:	return string.Format(NetSpy_AsmEditor_Resources.EditCodeAddClassMembers2, info.Value.decompiler.GenericNameUI);
