...
using dnlib.PE;
using NetSpy.AsmEditor.Event;
using NetSpy.AsmEditor.Field;
using NetSpy.AsmEditor.Method;
using NetSpy.AsmEditor.Properties;
using NetSpy.AsmEditor.Property;
using NetSpy.AsmEditor.Types;
using NetSpy.Contracts.AsmEditor.Compiler;
using NetSpy.Decompiler.Utils;
...
namespace NetSpy.AsmEditor.Compiler {
	...
	sealed partial class ModuleImporter {
		...
		TypeDef FindSourceType(TypeDef targetType) {
			...
			AddErrorThrow(IM0010, string.Format(NetSpy_AsmEditor_Resources.ERR_IM_CouldNotFindEditedType, targetType));
			throw new InvalidOperationException();
		}
		...
		MethodDef FindSourceMethod(MethodDef targetMethod) {
			...
			if (newType is null)
				AddErrorThrow(IM0001, string.Format(NetSpy_AsmEditor_Resources.ERR_IM_CouldNotFindMethodType, targetMethod.DeclaringType));
			...
			AddErrorThrow(IM0002, string.Format(NetSpy_AsmEditor_Resources.ERR_IM_CouldNotFindEditedMethod, targetMethod));
			throw new InvalidOperationException();
		}
		...
		void RenameMergedMembers(MergedImportedType mergedType) {
			...
			foreach (var compiledProp in compiledType.Properties) {
				...
				if (IsVirtual(compiledProp))
					AddError(IM0006, string.Format(NetSpy_AsmEditor_Resources.ERR_IM_RenamingVirtualPropsIsNotSupported, compiledProp));
			...
			foreach (var compiledEvent in compiledType.Events) {
				...
				if (IsVirtual(compiledEvent))
					AddError(IM0007, string.Format(NetSpy_AsmEditor_Resources.ERR_IM_RenamingVirtualEventsIsNotSupported, compiledEvent));
			...
			foreach (var compiledMethod in compiledType.Methods) {
				...
				if (compiledMethod.IsVirtual)
					AddError(IM0008, string.Format(NetSpy_AsmEditor_Resources.ERR_IM_RenamingVirtualMethodsIsNotSupported, compiledMethod));
				...
				foreach (var compiledMethod in toExtraData[importedType].CompiledType.Methods) {
					...
					if (editedMethodsToFix.TryGetValue(compiledMethod, out var targetMethod2)) {
						...
						if (compiledMethod.IsStatic != targetInfo.TargetMember.IsStatic)
							AddError(IM0009, string.Format(NetSpy_AsmEditor_Resources.ERR_IM_AddingRemovingStaticFromEditedMethodNotSupported, targetInfo.TargetMember));
		...
		TypeDef? TryGetTypeInTargetModule(ITypeDefOrRef? tdr, out ImportedType? importedType) {
			...
			if (tdr is TypeRef tr) {
				...
				if (IsTarget(tr2.ResolutionScope)) {
					...
					AddError(IM0003, string.Format(NetSpy_AsmEditor_Resources.ERR_IM_CouldNotFindType, tr));
					importedType = null;
					return null;
				}
		...
		IMethod? Import(IMethod? method) {
			...
			if (td is not null) {
				...
				AddError(IM0004, string.Format(NetSpy_AsmEditor_Resources.ERR_IM_CouldNotFindMethod, mr));
				return null;
			}
		...
		IField? Import(IField? field) {
			...
			if (td is not null) {
				...
				AddError(IM0005, string.Format(NetSpy_AsmEditor_Resources.ERR_IM_CouldNotFindField, mr));
				return null;
			}
