...
using System;
using dnlib.DotNet;
using NetSpy.AsmEditor.Compiler.MDEditor;
	...
	readonly struct ModulePatcher {
		...
		public unsafe bool Patch(ModuleDef module, out RawModuleBytes newModuleData) {
			...
			if (fixTypeDefRefs || addIVT) {
				NetSpyEventSource.Log.EditCodePatchModuleStart(module.Location);
				...
				NetSpyEventSource.Log.EditCodePatchModuleStop(module.Location);
			}
