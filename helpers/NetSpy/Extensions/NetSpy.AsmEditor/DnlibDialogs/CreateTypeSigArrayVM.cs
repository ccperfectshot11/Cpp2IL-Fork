		...
		public string? Title {
			get {
				if (!string.IsNullOrEmpty(title))
					return title;
				if (IsUnlimitedCount)
					return NetSpy_AsmEditor_Resources.Create_TypeSigs;
				else if (RequiredCount!.Value == 1)
					return NetSpy_AsmEditor_Resources.Create_TypeSig;
				else
					return string.Format(NetSpy_AsmEditor_Resources.Create_N_TypeSigs, RequiredCount.Value);
			}
		}
		string? title;
		...
		public string NumberOfTypesLeftString {
			get {
				...
				return numLeft == 0 ? string.Empty :
					numLeft == 1 ? NetSpy_AsmEditor_Resources.Create_TypeSigs_OneLeft :
					string.Format(NetSpy_AsmEditor_Resources.Create_TypeSigs_N_Left, numLeft);
			}
		}
