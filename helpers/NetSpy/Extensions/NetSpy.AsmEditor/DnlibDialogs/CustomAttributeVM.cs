...
namespace NetSpy.AsmEditor.DnlibDialogs {
	sealed class CustomAttributeVM : ViewModelBase {
		...
		public string FullName {
			get {
				if (IsRawData)
					return NetSpy_AsmEditor_Resources.RawCustomAttributeBlob2;
		...
		void PickConstructor() {
			...
			var newCtor = dnlibTypePicker.GetDnlibType(NetSpy_AsmEditor_Resources.Pick_Constructor, new FlagsDocumentTreeNodeFilter(VisibleMembersFlags.InstanceConstructor), Constructor, ownerModule);
