...
namespace NetSpy.AsmEditor.DnlibDialogs {
	struct EnumInfo {
		...
		public override string ToString() {
			...
			if (!IsArray)
				return $"({(EnumType ?? (object)NetSpy_AsmEditor_Resources.UnknownEnum)}){Value}";

			var list = Value as System.Collections.IList;
			if (list is null)
				return $"({(EnumType ?? (object)NetSpy_AsmEditor_Resources.UnknownEnum)}[])null";

			var sb = new StringBuilder();
			sb.Append($"new {(EnumType ?? (object)NetSpy_AsmEditor_Resources.UnknownEnum)}[] {{");
	...
	abstract class EnumDataFieldVMBase : DataFieldVM<EnumInfo> {
		...
		public string PickEnumToolTip {
			get {
				if (enumInfo.EnumType is null)
					return NetSpy_AsmEditor_Resources.Pick_EnumType;
				return string.Format(NetSpy_AsmEditor_Resources.EnumType, enumInfo.EnumType.FullName);
			}
		}
		...
		void PickEnumType() {
			...
			var type = dnlibTypePicker.GetDnlibType(NetSpy_AsmEditor_Resources.Pick_EnumType, new FlagsDocumentTreeNodeFilter(VisibleMembersFlags.EnumTypeDef), EnumType, ownerModule);
			if (type is not null)
				EnumType = type;
		}
