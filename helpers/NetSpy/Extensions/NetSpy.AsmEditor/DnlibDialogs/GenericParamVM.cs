...
namespace NetSpy.AsmEditor.DnlibDialogs {
	...
	sealed class GenericParamVM : ViewModelBase {
		...
		public string FullName {
			get {
				...
				if (Number.HasError)
					...
					sb.Append(string.Format(NetSpy_AsmEditor_Resources.GenericParameterNumber, Number.Value));
		...
		public GenericParamVM(GenericParamOptions options, ModuleDef ownerModule, IDecompilerService decompilerService, TypeDef? ownerType, MethodDef? ownerMethod) {
			...
			TypeDefOrRefAndCAsVM = new TypeDefOrRefAndCAsVM<GenericParamConstraint>(NetSpy_AsmEditor_Resources.EditGenericParameterConstraint, NetSpy_AsmEditor_Resources.CreateGenericParameterConstraint, ownerModule, decompilerService, ownerType, ownerMethod);
