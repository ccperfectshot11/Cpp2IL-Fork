...
namespace NetSpy.AsmEditor.DnlibDialogs {
	sealed class MemberRefVM : ViewModelBase {
		...
		public MemberRefVM(MemberRefOptions options, TypeSigCreatorOptions typeSigCreatorOptions, bool isField) {
			...
			TypeSigCreatorVM = new TypeSigCreatorVM(this.typeSigCreatorOptions.Clone(NetSpy_AsmEditor_Resources.CreateFieldTypeSig));
		...
		void InitializeFromField() {
			...
			var newField = dnlibTypePicker.GetDnlibType<IField>(NetSpy_AsmEditor_Resources.Pick_Field, new FlagsDocumentTreeNodeFilter(VisibleMembersFlags.FieldDef), null, typeSigCreatorOptions.OwnerModule);
		...
		void InitializeFromMethod() {
			if (dnlibTypePicker is null)
				throw new InvalidOperationException();
			var newMethod = dnlibTypePicker.GetDnlibType<IMethod>(NetSpy_AsmEditor_Resources.Pick_Method, new FlagsDocumentTreeNodeFilter(VisibleMembersFlags.MethodDef), null, typeSigCreatorOptions.OwnerModule);
		...
		void PickType() {
			if (dnlibTypePicker is null)
				throw new InvalidOperationException();
			var newType = dnlibTypePicker.GetDnlibType(NetSpy_AsmEditor_Resources.Pick_Type, new FlagsDocumentTreeNodeFilter(VisibleMembersFlags.TypeDef), Class as ITypeDefOrRef, typeSigCreatorOptions.OwnerModule);
		...
		void PickTypeSpec() {
			if (typeSigCreator is null)
				throw new InvalidOperationException();
			var newType = typeSigCreator.Create(typeSigCreatorOptions.Clone(NetSpy_AsmEditor_Resources.CreateTypeSpec), (Class as ITypeDefOrRef).ToTypeSig(), out bool canceled);
			if (!canceled)
		...
		void PickMethodDef() {
			if (dnlibTypePicker is null)
				throw new InvalidOperationException();
			var newMethod = dnlibTypePicker.GetDnlibType(NetSpy_AsmEditor_Resources.Pick_Method, new SameAssemblyDocumentTreeNodeFilter(typeSigCreatorOptions.OwnerModule, new FlagsDocumentTreeNodeFilter(VisibleMembersFlags.MethodDef)), Class as IMethod, typeSigCreatorOptions.OwnerModule);
			if (newMethod is not null) {
		...
		void PickModuleRef() {
			if (dnlibTypePicker is null)
				throw new InvalidOperationException();
			var document = dnlibTypePicker.GetDnlibType<IDsDocument>(NetSpy_AsmEditor_Resources.Pick_Module, new SameAssemblyDocumentTreeNodeFilter(typeSigCreatorOptions.OwnerModule, new FlagsDocumentTreeNodeFilter(VisibleMembersFlags.ModuleDef)), null, typeSigCreatorOptions.OwnerModule);
