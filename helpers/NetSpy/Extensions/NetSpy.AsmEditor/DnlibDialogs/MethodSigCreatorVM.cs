...
namespace NetSpy.AsmEditor.DnlibDialogs {
	...
	sealed class MethodSigCreatorVM : ViewModelBase {
		...
		public string? Title {
			get {
				if (!string.IsNullOrEmpty(title))
					return title;
				return IsPropertySig ? NetSpy_AsmEditor_Resources.CreatePropertySignature : NetSpy_AsmEditor_Resources.CreateMethodSignature;
			}
		}
		string? title;
		...
		void AddReturnType() {
			...
			var newTypeSig = typeSigCreator.Create(options.TypeSigCreatorOptions.Clone(NetSpy_AsmEditor_Resources.CreateReturnType), ReturnType, out bool canceled);
		...
		protected override string? Verify(string columnName) {
			if (columnName == nameof(ReturnType))
				return ReturnType is not null ? string.Empty : NetSpy_AsmEditor_Resources.ReturnTypeRequired;
			return string.Empty;
		}
