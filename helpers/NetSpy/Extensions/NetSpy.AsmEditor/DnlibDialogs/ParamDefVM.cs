...
namespace NetSpy.AsmEditor.DnlibDialogs {
	sealed class ParamDefVM : ViewModelBase {
		...
		public string FullName {
			get {
				...
				if (Sequence.HasError)
					...
					sb.Append(NetSpy_AsmEditor_Resources.ParameterReturn);
				else
					sb.Append(string.Format(NetSpy_AsmEditor_Resources.ParameterNumber, Sequence.Value));

				sb.Append(' ');
				sb.Append(string.IsNullOrEmpty(Name) ? NetSpy_AsmEditor_Resources.NoName : Name);
		...
		public string MarshalTypeString => string.Format(NetSpy_AsmEditor_Resources.MarshalType, HasFieldMarshal ? MarshalTypeVM.TypeString : NetSpy_AsmEditor_Resources.MarshalType_Nothing);
		...
		public ParamDefVM(ParamDefOptions options, ModuleDef ownerModule, IDecompilerService decompilerService, TypeDef? ownerType, MethodDef? ownerMethod) {
			...
			ConstantVM = new ConstantVM(ownerModule, options.Constant?.Value, NetSpy_AsmEditor_Resources.Parameter_DefaultValueInfo);
