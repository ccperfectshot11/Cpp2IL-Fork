	...
	sealed class TypeSigCreatorVM : ViewModelBase {
		...
		public string Title {
			get {
				...
				return NetSpy_AsmEditor_Resources.CreateTypeSig;
			}
		}
		...
		public bool IsValidTypeSig => options.NullTypeSigAllowed || TypeSig is not null;
		public bool CanAddLeafTypeSig => IsEnabled && TypeSig is null;
		public bool AddingLeafTypeSig => TypeSig is null;
		public bool CanAddNonLeafTypeSig => IsEnabled && !(TypeSig is PinnedSig) && TypeSig is not null;
		public bool AddingNonLeafTypeSig => TypeSig is not null;
		...
		public ICommand ClearTypeSigCommand => new RelayCommand(a => TypeSig = null, a => IsEnabled && TypeSig is not null);
		public ICommand RemoveLastTypeSigCommand => new RelayCommand(a => RemoveLastTypeSig(), a => IsEnabled && TypeSig is not null);
		...
		void RemoveLastTypeSig() {
			if (TypeSig is not null)
				TypeSig = TypeSig.Next;
		}

		TypeDefOrRefSig? GetTypeSig() => GetTypeSig(NetSpy_AsmEditor_Resources.Pick_Type, VisibleMembersFlags.TypeDef);
		...
		void AddFnPtrSig() {
			...
			var createOptions = new MethodSigCreatorOptions(options.Clone(NetSpy_AsmEditor_Resources.CreateFnPtrMethodSignature));
		...
		void AddGenericInstSig() {
			if (createTypeSigArray is null)
				throw new InvalidOperationException();
			var origType = GetTypeSig(NetSpy_AsmEditor_Resources.Pick_GenericType, VisibleMembersFlags.GenericTypeDef);
			if (origType is null)
				return;
			var type = origType as ClassOrValueTypeSig;
			if (type is null) {
				ShowWarning(null, NetSpy_AsmEditor_Resources.TypeMustBeGeneric);
				return;
			}
			var genericType = type.TypeDefOrRef.ResolveTypeDef();
			if (genericType is null) {
				ShowWarning(null, NetSpy_AsmEditor_Resources.CouldNotResolveType);
				return;
			}
			if (genericType.GenericParameters.Count == 0) {
				ShowWarning(null, string.Format(NetSpy_AsmEditor_Resources.NotGenericType, genericType.FullName));
				return;
			}

			var genArgs = createTypeSigArray.Create(options.Clone(NetSpy_AsmEditor_Resources.CreateGenericInstanceTypeArguments), genericType.GenericParameters.Count, null);
			if (genArgs is null)
				return;
