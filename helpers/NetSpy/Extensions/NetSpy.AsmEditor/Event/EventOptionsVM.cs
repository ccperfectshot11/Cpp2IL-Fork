...
namespace NetSpy.AsmEditor.Event {
	sealed class EventOptionsVM : ViewModelBase {
		...
		public string EventTypeHeader => string.Format(NetSpy_AsmEditor_Resources.EventType, TypeSigCreator.TypeSigDnlibFullName);
		...
		MethodDef? PickMethod(MethodDef? origMethod) {
			...
			return dnlibTypePicker.GetDnlibType(NetSpy_AsmEditor_Resources.Pick_Method, new SameModuleDocumentTreeNodeFilter(ownerModule, new FlagsDocumentTreeNodeFilter(VisibleMembersFlags.MethodDef)), origMethod, ownerModule);
		}
