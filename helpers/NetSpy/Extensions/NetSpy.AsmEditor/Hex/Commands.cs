...
using dnlib.DotNet;
using NetSpy.AsmEditor.Hex.Nodes;
using NetSpy.AsmEditor.Properties;
using NetSpy.AsmEditor.Utilities;
...
namespace NetSpy.AsmEditor.Hex {
	...
	static class GoToMDTableRowHexEditorCommand {
		...
		static string GetHeaderInternal(IDocumentTabService documentTabService, HexContext context) {
			...
			return string.Format(NetSpy_AsmEditor_Resources.GoToMetaDataTableRowCommand, tokRef.Token);
		}
		...
		static string? GetInputGestureTextInternal(HexContext context) {
			if (context.CreatorObject.Guid == new Guid(MenuConstants.GUIDOBJ_DOCUMENTS_TREEVIEW_GUID) || context.CreatorObject.Guid == new Guid(MenuConstants.GUIDOBJ_DOCUMENTVIEWERCONTROL_GUID))
				return NetSpy_AsmEditor_Resources.ShortCutKeyShiftAltR;
			return null;
		}
	...
	static class GoToMDTableRowUIHexEditorCommand {
		...
		static void Execute2(IDocumentTabService documentTabService, HexContext context) {
			...
			uint? token = AskForDef(NetSpy_AsmEditor_Resources.GoToMetaDataTableRowTitle, module);
			if (token is null)
				return;
			...
			if (HexDocumentTreeNodeDataFinder.FindNode(documentTabService.DocumentTreeView, tokRef) is null) {
				MsgBox.Instance.Show(string.Format(NetSpy_AsmEditor_Resources.GoToMetaDataTableRow_TokenDoesNotExist, token.Value));
				return;
			}
		...
		static uint? AskForDef(string title, ModuleDef module) => MsgBox.Instance.Ask(NetSpy_AsmEditor_Resources.GoToMetaDataTableRow_MetadataToken, null, title, s => {
			...
			return string.Format(NetSpy_AsmEditor_Resources.GoToMetaDataTableRow_InvalidMetadataToken, token);
		});
	}
}
