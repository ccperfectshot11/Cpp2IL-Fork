...
namespace NetSpy.AsmEditor.Hex {
	...
	static class GoToRidMDTableCommand {
		...
		static void ExecuteInternal(MDTableContext context) {
			var recVM = Ask(NetSpy_AsmEditor_Resources.GoToRowIdentifier_Title, context);
			if (recVM is not null)
				UIUtils.ScrollSelectAndSetFocus(context.ListView, recVM);
		}

		static MetadataTableRecordVM? Ask(string title, MDTableContext context) => MsgBox.Instance.Ask(NetSpy_AsmEditor_Resources.GoToMetaDataTableRow_RID, null, title, s => {
			...
			if (rid == 0 || rid > context.MetadataTableVM.Rows)
				return string.Format(NetSpy_AsmEditor_Resources.GoToRowIdentifier_InvalidRowIdentifier, rid);
			return string.Empty;
		});
	}
	...
	static class CopyMDTableCommand {
		...
		static void ExecuteInternal(MDTableContext context) {
			...
			if (totalSize >= int.MaxValue) {
				MsgBox.Instance.Show(NetSpy_AsmEditor_Resources.TooManyBytesSelected);
				return;
			}
	...
	static class PasteMDTableCommand {
		...
		static string? GetHeaderInternal(MDTableContext context) {
			...
			if (recs <= 1)
				return null;
			return string.Format(NetSpy_AsmEditor_Resources.PasteRecordsCommand, recs, context.Records[0].Span.Start.ToUInt64(), context.Records[0].Token.Rid);
		}
	}
}
