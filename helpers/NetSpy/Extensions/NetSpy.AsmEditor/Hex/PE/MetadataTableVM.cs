...
namespace NetSpy.AsmEditor.Hex.PE {
	abstract class MetadataTableVM : HexVM {
		...
		public string InfoName => NetSpy_AsmEditor_Resources.Info;
		...
		static TableInfo CreateTableInfo(TableInfo info) {
			...
			for (int i = 0, coli = 0; i < info.Columns.Count; i++, coli++) {
				...
				for (int j = 0; j < padding; j++) {
					newCols.Add(new ColumnInfo((byte)(coli++ + 1), NetSpy_AsmEditor_Resources.Padding, ColumnSize.Byte, (byte)offs, 1));
					offs++;
				}
			}
