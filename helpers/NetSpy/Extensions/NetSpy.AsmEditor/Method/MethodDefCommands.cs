...
namespace NetSpy.AsmEditor.Method {
	[ExportAutoLoaded]
	...
	sealed class DeleteMethodDefCommand : IUndoCommand {
		...
		static string GetHeader(DocumentTreeNodeData[] nodes) {
			if (nodes.Length == 1)
				return string.Format(NetSpy_AsmEditor_Resources.DeleteX, UIUtilities.EscapeMenuItemHeader(UIUtilities.TruncateWithElipsis(nodes[0].ToString())));
			return string.Format(NetSpy_AsmEditor_Resources.DeleteMethodsCommand, nodes.Length);
		}
		...
		static void Execute(Lazy<IUndoCommandService> undoCommandService, DocumentTreeNodeData[] nodes) {
			...
			if (!AskDeleteDef(NetSpy_AsmEditor_Resources.AskDeleteMethod))
				return;
		...
		public string Description => NetSpy_AsmEditor_Resources.DeleteMethodCommand;
	...
	sealed class CreateMethodDefCommand : IUndoCommand {
		...
		static void Execute(Lazy<IUndoCommandService> undoCommandService, IAppService appService, DocumentTreeNodeData[] nodes) {
			...
			win.Title = NetSpy_AsmEditor_Resources.CreateMethodCommand2;
		...
		public string Description => NetSpy_AsmEditor_Resources.CreateMethodCommand2;
	...
	sealed class MethodDefSettingsCommand : IUndoCommand {
		...
		public string Description => NetSpy_AsmEditor_Resources.EditMethodCommand2;
