...
namespace NetSpy.AsmEditor.MethodBody {
	sealed class ExceptionHandlerVM : ViewModelBase, IIndexedItem {
		...
		public ExceptionHandlerVM(TypeSigCreatorOptions typeSigCreatorOptions, ExceptionHandlerOptions options) {
			this.typeSigCreatorOptions = typeSigCreatorOptions.Clone(NetSpy_AsmEditor_Resources.CreateAnExceptionCatchType);
		...
		string VerifyInstruction(ListVM<InstructionVM> list) {
			...
			if (item is not null && instr is null)
				return NetSpy_AsmEditor_Resources.Error_OnlyInstrsCanBeSelected;

			if (instr is not null && instr.Index == -1)
				return NetSpy_AsmEditor_Resources.Error_InstrHasBeenRemoved;
