		...
		string VerifyOperand(ListVM<object> list) {
			...
			case MethodBody.InstructionOperandType.BranchTarget:
				...
				if (instr is null || instr == InstructionVM.Null)
					return NetSpy_AsmEditor_Resources.Error_OpMustBeInstr;
				if (instr.Index == -1)
					return NetSpy_AsmEditor_Resources.Error_InstrTargetHasBeenRemoved;
				break;

			case MethodBody.InstructionOperandType.Local:
				var local = item as LocalVM;
				if (local is null || local == LocalVM.Null)
					return NetSpy_AsmEditor_Resources.Error_OpMustBeLocal;
				if (local.Index == -1)
					return NetSpy_AsmEditor_Resources.Error_LocalHasBeenRemoved;
				break;

			case MethodBody.InstructionOperandType.Parameter:
				var p = item as Parameter;
				if (p is null || p == BodyUtils.NullParameter)
					return NetSpy_AsmEditor_Resources.Error_OpMustBeParam;
				break;
			...
			if (columnName == nameof(Other)) {
				...
				case MethodBody.InstructionOperandType.Field:
					if (Other is not null) {
						if (!(Other is IField))
							return NetSpy_AsmEditor_Resources.Error_OpMustBeField;
				...
				case MethodBody.InstructionOperandType.Method:
					if (Other is not null) {
						if (!(Other is IMethod))
							return NetSpy_AsmEditor_Resources.Error_OpMustBeMethod;
				...
				case MethodBody.InstructionOperandType.Token:
					if (Other is not null && !(Other is ITokenOperand)) return NetSpy_AsmEditor_Resources.Error_OpMustBeTypeMethodField;
					break;

				case MethodBody.InstructionOperandType.Type:
					if (Other is not null && !(Other is ITypeDefOrRef)) return NetSpy_AsmEditor_Resources.Error_OpMustBeType;
					break;

				case MethodBody.InstructionOperandType.MethodSig:
					if (Other is not null && !(Other is MethodSig)) return NetSpy_AsmEditor_Resources.Error_OpMustBeMethodSig;
					break;

				case MethodBody.InstructionOperandType.SwitchTargets:
					var list = Other as IList<InstructionVM>;
					if (list is null)
						return NetSpy_AsmEditor_Resources.Error_OpMustBeListInstrs;
					foreach (var i in list) {
						if (i is not null && i.Index == -1)
							return NetSpy_AsmEditor_Resources.Error_SwitchInstrTargetHasBeenRemoved;
					}
					break;
