...
namespace NetSpy.AsmEditor.MethodBody {
	sealed class LocalsListHelper : ListBoxHelperBase<LocalVM> {
		...
		protected override string AddNewBeforeSelectionMessage => NetSpy_AsmEditor_Resources.Local_Command1;
		protected override string AddNewAfterSelectionMessage => NetSpy_AsmEditor_Resources.Local_Command2;
		protected override string AppendNewMessage => NetSpy_AsmEditor_Resources.Local_Command3;
		protected override string RemoveSingularMessage => NetSpy_AsmEditor_Resources.Local_Command4;
		protected override string RemovePluralMessage => NetSpy_AsmEditor_Resources.Local_Command5;
		protected override string RemoveAllMessage => NetSpy_AsmEditor_Resources.Local_Command6;
		...
		protected override void CopyItemsAsText(LocalVM[] locals) {
			...
			for (int i = 0; i < locals.Length; i++) {
				...
				output.Write(BoxedTextColor.Text, local.IsPinned ? NetSpy_AsmEditor_Resources.Local_Pinned_Character : string.Empty);
				output.Write(BoxedTextColor.Text, "\t");
				output.Write(BoxedTextColor.Text, local.DebuggerHidden ? NetSpy_AsmEditor_Resources.Local_CompilerGenerated_Character : string.Empty);
				output.Write(BoxedTextColor.Text, "\t");
