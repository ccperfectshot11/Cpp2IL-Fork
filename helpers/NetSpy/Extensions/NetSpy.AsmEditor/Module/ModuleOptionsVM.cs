...
namespace NetSpy.AsmEditor.Module {
	...
	sealed class ModuleOptionsVM : ViewModelBase {
		...
		public ModuleOptionsVM(ModuleDef module, ModuleOptions options, IDecompilerService decompilerService) {
			...
			ClrVersion.Items.Add(new EnumVM(Module.ClrVersion.Unknown, NetSpy_AsmEditor_Resources.Unknown));
		...
		void PickManagedEntryPoint() {
			if (dnlibTypePicker is null)
				throw new InvalidOperationException();
			var ep = dnlibTypePicker.GetDnlibType(NetSpy_AsmEditor_Resources.Pick_EntryPoint, new EntryPointDocumentTreeNodeFilter(module), ManagedEntryPoint, module);
			if (ep is not null) {
