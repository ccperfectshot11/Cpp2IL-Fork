...
namespace NetSpy.AsmEditor.Property {
	sealed class PropertyOptionsVM : ViewModelBase {
		...
		public string PropertySigHeader => string.Format(NetSpy_AsmEditor_Resources.PropertyX, MethodSigCreator.HasError ? "null" : MethodSigCreator.PropertySig!.ToString());
		...
		public PropertyOptionsVM(PropertyDefOptions options, ModuleDef ownerModule, IDecompilerService decompilerService, TypeDef ownerType) {
			...
			ConstantVM = new ConstantVM(ownerModule, options.Constant is null ? null : options.Constant.Value, NetSpy_AsmEditor_Resources.Property_DefaultValue);
