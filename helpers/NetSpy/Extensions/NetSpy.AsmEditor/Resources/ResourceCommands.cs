...
namespace NetSpy.AsmEditor.Resources {
	...
	sealed class DeleteResourceCommand : IUndoCommand {
		...
		static string GetHeader(DocumentTreeNodeData[] nodes) {
			if (nodes.Length == 1)
				return string.Format(NetSpy_AsmEditor_Resources.DeleteX, UIUtilities.EscapeMenuItemHeader(nodes[0].ToString()));
			return string.Format(NetSpy_AsmEditor_Resources.DeleteResourcesCommand, nodes.Length);
		}
		...
		public string Description => NetSpy_AsmEditor_Resources.DeleteResourceCommand;
	...
	static class SaveResourcesCommand {
		...
		static string GetHeaderInternal(ResourceData[] infos) {
			if (infos.Length == 1)
				return string.Format(NetSpy_AsmEditor_Resources.SaveResourceCommand, UIUtilities.EscapeMenuItemHeader(infos[0].Name));
			return string.Format(NetSpy_AsmEditor_Resources.SaveResourcesCommand, infos.Length);
		}
	}
	...
	static class SaveWithPathResourcesCommand {
		...
		static string GetHeaderInternal(ResourceData[] infos) => string.Format(NetSpy_AsmEditor_Resources.SaveResourcesSubDirectoriesCommand, infos.Length);
	...
	static class SaveRawResourcesCommand {
		...
		static string GetHeaderInternal(ResourceData[] infos) {
			if (infos.Length == 1)
				return string.Format(NetSpy_AsmEditor_Resources.RawSaveResourceCommand, UIUtilities.EscapeMenuItemHeader(infos[0].Name));
			return string.Format(NetSpy_AsmEditor_Resources.RawSaveResourcesCommand, infos.Length);
		}
	}
	...
	static class SaveRawWithPathResourcesCommand {
		...
		static string GetHeaderInternal(ResourceData[] infos) => string.Format(NetSpy_AsmEditor_Resources.RawSaveResourcesSubDirectoriesCommand, infos.Length);
	...
	sealed class CreateFileResourceCommand : IUndoCommand {
		...
		static void Execute(Lazy<IUndoCommandService> undoCommandService, IAppService appService, IResourceNodeFactory resourceNodeFactory, IPickFilename pickFilename, DocumentTreeNodeData[] nodes) {
			...
			for (int i = 0; i < fnames.Length; i++) {
				...
				try {
					...
					MsgBox.Instance.Show(string.Format(NetSpy_AsmEditor_Resources.Error_ReadingFiles, ex.Message));
					return;
				}
			}
		...
		public string Description => NetSpy_AsmEditor_Resources.CreateFileResourceCommand2;
	...
	sealed class CreateMultiFileResourceCommand : CreateResourceTreeNodeCommand {
		...
		static void Execute(Lazy<IUndoCommandService> undoCommandService, IAppService appService, IResourceNodeFactory resourceNodeFactory, DocumentTreeNodeData[] nodes) {
			...
			win.Title = NetSpy_AsmEditor_Resources.CreateMultiFileResourceCommand2;
		...
		public override string Description => NetSpy_AsmEditor_Resources.CreateMultiFileResourceCommand2;
	}
	...
	sealed class CreateAssemblyLinkedResourceCommand : CreateResourceTreeNodeCommand {
		...
		static void Execute(Lazy<IUndoCommandService> undoCommandService, IAppService appService, IResourceNodeFactory resourceNodeFactory, DocumentTreeNodeData[] nodes) {
			...
			var data = new ResourceVM(options, module, appService.DecompilerService);
			var win = new ResourceDlg();
			win.Title = NetSpy_AsmEditor_Resources.CreateAssemblyLinkedResourceCommand2;
			win.DataContext = data;
			win.Owner = appService.MainWindow;
			if (win.ShowDialog() != true)
				return;
		...
		public override string Description => NetSpy_AsmEditor_Resources.CreateAssemblyLinkedResourceCommand2;
	}
	...
	sealed class CreateFileLinkedResourceCommand : CreateResourceTreeNodeCommand {
		...
		static void Execute(Lazy<IUndoCommandService> undoCommandService, IAppService appService, IResourceNodeFactory resourceNodeFactory, DocumentTreeNodeData[] nodes) {
			...
			var data = new ResourceVM(options, module, appService.DecompilerService);
			var win = new ResourceDlg();
			win.Title = NetSpy_AsmEditor_Resources.CreateFileLinkedResourceCommand2;
			win.DataContext = data;
			win.Owner = appService.MainWindow;
			if (win.ShowDialog() != true)
				return;
		...
		public override string Description => NetSpy_AsmEditor_Resources.CreateFileLinkedResourceCommand2;
	}
	...
	sealed class ResourceSettingsCommand : IUndoCommand {
		...
		public string Description => NetSpy_AsmEditor_Resources.EditResourceCommand2;
	...
	sealed class CreateImageResourceElementCommand : CreateResourceElementCommandBase {
		...
		static void Execute(Lazy<IUndoCommandService> undoCommandService, IAppService appService, IResourceNodeFactory resourceNodeFactory, IPickFilename pickFilename, DocumentTreeNodeData[] nodes) {
			...
			for (int i = 0; i < fnames.Length; i++) {
				...
				try {
					...
					if (error is null)
						error = string.Format(NetSpy_AsmEditor_Resources.Error_ReadingFiles, ex.Message);
				}
			}
		...
		public override string Description => NetSpy_AsmEditor_Resources.CreateBitMapIconResourceCommand2;
	}
	...
	sealed class CreateImageListResourceElementCommand : CreateResourceElementCommandBase {
		...
		static void Execute(Lazy<IUndoCommandService> undoCommandService, IAppService appService, IResourceNodeFactory resourceNodeFactory, DocumentTreeNodeData[] nodes) {
			...
			win.Title = NetSpy_AsmEditor_Resources.CreateImageListStreamerResourceCommand2;
			win.DataContext = data;
			win.Owner = appService.MainWindow;
			if (win.ShowDialog() != true)
				return;
			...
			if (data.ImageListStreamerVM.Collection.Count == 0) {
				MsgBox.Instance.Show(NetSpy_AsmEditor_Resources.Error_EmptyImageList);
				return;
			}
			...
			try {
				...
				error = string.Format(NetSpy_AsmEditor_Resources.Error_CouldNotSerializeImages, ex.Message);
			}
		...
		public override string Description => NetSpy_AsmEditor_Resources.CreateImageListStreamerResourceCommand2;
	}
	...
	sealed class CreateByteArrayResourceElementCommand : CreateResourceElementCommandBase {
		...
		public override string Description => NetSpy_AsmEditor_Resources.CreateByteArrayResourceCommand2;
	}
	...
	sealed class CreateStreamResourceElementCommand : CreateResourceElementCommandBase {
		...
		public override string Description => NetSpy_AsmEditor_Resources.CreateStreamResourceCommand2;
	}
	...
	sealed class CreateResourceElementCommand : CreateResourceElementCommandBase {
		...
		static void Execute(Lazy<IUndoCommandService> undoCommandService, IAppService appService, IResourceNodeFactory resourceNodeFactory, IDocumentTreeViewSettings documentTreeViewSettings, DocumentTreeNodeData[] nodes) {
			...
			win.Title = NetSpy_AsmEditor_Resources.CreateResourceCommand2;
			win.DataContext = data;
			win.Owner = appService.MainWindow;
			if (win.ShowDialog() != true)
				return;
		...
		public override string Description => NetSpy_AsmEditor_Resources.CreateResourceCommand2;
	}
	...
	sealed class ResourceElementSettingsCommand : ResourceElementSettingsBaseCommand {
		...
		static void Execute(Lazy<IUndoCommandService> undoCommandService, IAppService appService, IDocumentTreeViewSettings documentTreeViewSettings, DocumentTreeNodeData[] nodes) {
			...
			var win = new ResourceElementDlg();
			win.Title = NetSpy_AsmEditor_Resources.EditResourceCommand2;
			win.DataContext = data;
			win.Owner = appService.MainWindow;
			if (win.ShowDialog() != true)
				return;
			...
			try {
				...
				error = string.Format(NetSpy_AsmEditor_Resources.Error_InvalidResourceData, ex.Message);
			}
		...
		public override string Description => NetSpy_AsmEditor_Resources.EditResourceCommand2;
	}
	...
	sealed class ImageResourceElementSettingsCommand : ResourceElementSettingsBaseCommand {
		...
		static void Execute(Lazy<IUndoCommandService> undoCommandService, IAppService appService, DocumentTreeNodeData[] nodes) {
			...
			try {
				...
				error = string.Format(NetSpy_AsmEditor_Resources.Error_NewResourceDataMustBeImage, ex.Message);
			}
			if (!string2.IsNullOrEmpty(error)) {
				MsgBox.Instance.Show(error);
				return;
