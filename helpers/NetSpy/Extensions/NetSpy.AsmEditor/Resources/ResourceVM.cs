...
namespace NetSpy.AsmEditor.Resources {
	...
	sealed class ResourceVM : ViewModelBase {
		...
		void PickAssembly() {
			...
			var newAsm = dnlibTypePicker.GetDnlibType<IDsDocument>(NetSpy_AsmEditor_Resources.Pick_Assembly, new FlagsDocumentTreeNodeFilter(VisibleMembersFlags.AssemblyDef), null, ownerModule);
		...
		protected override string? Verify(string columnName) {
			if (columnName == nameof(AssemblyFullName)) {
				if (Assembly is null)
					return NetSpy_AsmEditor_Resources.Error_AssemblyFieldMustNotBeEmpty;
				return string.Empty;
			}
			return string.Empty;
		}
