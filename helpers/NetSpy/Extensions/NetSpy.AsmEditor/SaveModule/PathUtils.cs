	...
	static class PathUtils {
		...
		public static string? ValidateFileName(this string filename) {
			if (string.IsNullOrEmpty(filename))
				return NetSpy_AsmEditor_Resources.Error_Filename_Empty;
			if (string.IsNullOrWhiteSpace(filename))
				return NetSpy_AsmEditor_Resources.Error_Filename_OnlyWhitespace;

			if (filename.Length >= 260)
				return NetSpy_AsmEditor_Resources.Error_Filename_TooLong;

			if (filename.IndexOfAny(Path.GetInvalidPathChars()) >= 0)
				return NetSpy_AsmEditor_Resources.Error_Filename_ContainsInvalidPathCharacter;
			...
			foreach (var invalidName in InvalidFileNamesNoExtension) {
				if (invalidName.Equals(fnameNoExt, StringComparison.OrdinalIgnoreCase))
					return NetSpy_AsmEditor_Resources.Error_Filename_ReservedOSFilename;
			}

			// Don't check for dirs on the network. Could take a while and will block the UI
			if (!filename.StartsWith(@"\\")) {
				if (Directory.Exists(filename))
					return NetSpy_AsmEditor_Resources.Error_Filename_FolderExists;
				var dir = Path.GetDirectoryName(filename);
				if (!string.IsNullOrEmpty(dir) && !Directory.Exists(dir))
					return string.Format(NetSpy_AsmEditor_Resources.Error_Filename_FolderDoesNotExist, dir);
			}
