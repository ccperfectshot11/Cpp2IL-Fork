...
namespace NetSpy.AsmEditor.SaveModule {
	class SaveModuleWindow : WindowBase {
		...
		protected override void OnClosing(CancelEventArgs e) {
			...
			var data = (SaveMultiModuleVM)DataContext;
			if (data.IsSaving) {
				var res = MsgBox.Instance.Show(NetSpy_AsmEditor_Resources.AskCancelSave, MsgBoxButton.Yes | MsgBoxButton.No);
			...
			if (data.IsCanceling) {
				var res = MsgBox.Instance.Show(NetSpy_AsmEditor_Resources.AskCancelSaveCloseWindow, MsgBoxButton.Yes | MsgBoxButton.No);
				if (res != MsgBoxButton.Yes)
					e.Cancel = true;
				return;
			}
