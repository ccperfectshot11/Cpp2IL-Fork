...
namespace NetSpy.AsmEditor.Types {
	...
	sealed class DeleteTypeDefCommand : IUndoCommand {
		...
		static string GetHeader(DocumentTreeNodeData[] nodes) {
			...
			if (nodes.Length == 1)
				return string.Format(NetSpy_AsmEditor_Resources.DeleteX, UIUtilities.TruncateWithElipsis(UIUtilities.EscapeMenuItemHeader(nodes[0].ToString())));
			return string.Format(NetSpy_AsmEditor_Resources.DeleteTypesCommand, nodes.Length);
		}
		...
		static void Execute(Lazy<IUndoCommandService> undoCommandService, DocumentTreeNodeData[] nodes) {
			...
			if (!Method.DeleteMethodDefCommand.AskDeleteDef(NetSpy_AsmEditor_Resources.AskDeleteType))
				return;
		...
		public string Description => NetSpy_AsmEditor_Resources.DeleteTypeCommand;
	...
	sealed class CreateTypeDefCommand : IUndoCommand {
		...
		static void Execute(Lazy<IUndoCommandService> undoCommandService, IAppService appService, DocumentTreeNodeData[] nodes) {
			...
			win.Title = NetSpy_AsmEditor_Resources.CreateTypeCommand2;
		...
		public string Description => NetSpy_AsmEditor_Resources.CreateTypeCommand2;
	...
	sealed class CreateNestedTypeDefCommand : IUndoCommand {
		...
		static void Execute(Lazy<IUndoCommandService> undoCommandService, IAppService appService, DocumentTreeNodeData[] nodes) {
			...
			var data = new TypeOptionsVM(options, module, appService.DecompilerService, null);
			var win = new TypeOptionsDlg();
			win.Title = NetSpy_AsmEditor_Resources.CreateNestedTypeCommand2;
			win.DataContext = data;
			win.Owner = appService.MainWindow;
			if (win.ShowDialog() != true)
				return;
		...
		public string Description => NetSpy_AsmEditor_Resources.CreateNestedTypeCommand2;
	...
	sealed class TypeDefSettingsCommand : IUndoCommand {
		...
		public string Description => NetSpy_AsmEditor_Resources.EditTypeCommand2;
