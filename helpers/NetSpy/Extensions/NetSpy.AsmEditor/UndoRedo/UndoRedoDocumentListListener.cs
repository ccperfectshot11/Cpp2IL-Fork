...
namespace NetSpy.AsmEditor.UndoRedo {
	...
	sealed class UndoRedoDocumentListListener : IDocumentListListener {
		...
		public bool CheckCanLoad(bool isReload) {
			...
			var question = isReload ? NetSpy_AsmEditor_Resources.AskReloadAssembliesLoseChanges :
						NetSpy_AsmEditor_Resources.AskLoadAssembliesLoseChanges;

			var msg = count == 1 ? NetSpy_AsmEditor_Resources.UnsavedFile : string.Format(NetSpy_AsmEditor_Resources.UnsavedFiles, count);
