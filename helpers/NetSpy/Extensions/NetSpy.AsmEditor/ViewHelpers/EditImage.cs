...
using NetSpy.AsmEditor.Resources;
	...
	sealed class EditImage : IEdit<ImageVM> {
		...
		public ImageVM? Edit(string? title, ImageVM mo) {
			var dlg = new WF.OpenFileDialog {
				...
				Multiselect = false,
				Filter = $"{NetSpy_AsmEditor_Resources.Images}|*.png;*.gif;*.bmp;*.dib;*.jpg;*.jpeg;*.jpe;*.jif;*.jfif;*.jfi;*.ico|{NetSpy_AsmEditor_Resources.AllFiles} (*.*)|*.*",
			};
			...
			try {
				...
				if (imgStream is not null)
					imgStream.Dispose();
				MsgBox.Instance.Show(string.Format(NetSpy_AsmEditor_Resources.Error_OpenImage, ex.Message), MsgBoxButton.OK, ownerWindow);
				return null;
			}
		}
	}
