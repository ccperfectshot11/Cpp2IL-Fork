...
namespace NetSpy.AsmEditor.ViewHelpers {
	sealed class EditMethodOverride : IEdit<MethodOverrideVM> {
		...
		public MethodOverrideVM? Edit(string? title, MethodOverrideVM mo) {
			...
			var method = dnlibPicker.GetDnlibType<IMethodDefOrRef>(NetSpy_AsmEditor_Resources.Pick_Method, new FlagsDocumentTreeNodeFilter(VisibleMembersFlags.MethodDef), mo.MethodDeclaration, mo.OwnerModule);
			if (method is null)
				return null;
