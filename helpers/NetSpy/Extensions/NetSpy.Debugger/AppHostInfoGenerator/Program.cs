...
namespace AppHostInfoGenerator {
	sealed class Program : IDisposable {
		...
		static int Main(string[] args) {
			...
			switch (args.Length) {
			case 0:
				filename = Path.GetFullPath(Path.Combine(Path.GetDirectoryName(typeof(Program).Assembly.Location)!, "..", "..", "..", "..", "NetSpy.Debugger.DotNet.CorDebug", "Impl", defaultFilename));
				break;
			case 1:
				filename = args[0];
				break;
		...
		int DoIt() {
			try {
				...
				output.WriteLine();
				output.WriteLine("    This file is part of NetSpy");
				output.WriteLine();
				output.WriteLine("    NetSpy is free software: you can redistribute it and/or modify");
				...
				output.WriteLine();
				output.WriteLine("    NetSpy is distributed in the hope that it will be useful,");
				...
				output.WriteLine("    along with NetSpy.  If not, see <http://www.gnu.org/licenses/>.");
				output.WriteLine("*/");
				output.WriteLine();
				...
				output.WriteLine();
				output.WriteLine("namespace NetSpy.Debugger.DotNet.CorDebug.Impl {");
