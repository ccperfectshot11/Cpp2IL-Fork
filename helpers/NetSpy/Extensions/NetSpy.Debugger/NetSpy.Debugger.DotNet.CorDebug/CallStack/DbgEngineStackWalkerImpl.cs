...
namespace NetSpy.Debugger.DotNet.CorDebug.CallStack {
	sealed class DbgEngineStackWalkerImpl : DbgEngineStackWalker {
		...
		DbgEngineStackFrame CreateEngineStackFrame(CorFrame corFrame) {
			...
			if (corFrame.IsNativeFrame) {
				var name = string.Format(NetSpy_Debugger_DotNet_CorDebug_Resources.StackFrame_NativeFrame, "0x" + corFrame.NativeFrameIP.ToString("X8"));
		...
		static string GetInternalFrameTypeName(CorFrame corFrame) {
			...
			case CorDebugInternalFrameType.STUBFRAME_M2U:
				return NetSpy_Debugger_DotNet_CorDebug_Resources.StackFrame_ManagedToNativeTransition;

			case CorDebugInternalFrameType.STUBFRAME_U2M:
				return NetSpy_Debugger_DotNet_CorDebug_Resources.StackFrame_NativeToManagedTransition;

			case CorDebugInternalFrameType.STUBFRAME_APPDOMAIN_TRANSITION:
				return NetSpy_Debugger_DotNet_CorDebug_Resources.StackFrame_AppdomainTransition;

			case CorDebugInternalFrameType.STUBFRAME_LIGHTWEIGHT_FUNCTION:
				return NetSpy_Debugger_DotNet_CorDebug_Resources.StackFrame_LightweightFunction;

			case CorDebugInternalFrameType.STUBFRAME_FUNC_EVAL:
				return NetSpy_Debugger_DotNet_CorDebug_Resources.StackFrame_FunctionEvaluation;

			case CorDebugInternalFrameType.STUBFRAME_INTERNALCALL:
				return NetSpy_Debugger_DotNet_CorDebug_Resources.StackFrame_InternalCall;

			case CorDebugInternalFrameType.STUBFRAME_CLASS_INIT:
				return NetSpy_Debugger_DotNet_CorDebug_Resources.StackFrame_ClassInit;

			case CorDebugInternalFrameType.STUBFRAME_EXCEPTION:
				return NetSpy_Debugger_DotNet_CorDebug_Resources.StackFrame_Exception;

			case CorDebugInternalFrameType.STUBFRAME_SECURITY:
				return NetSpy_Debugger_DotNet_CorDebug_Resources.StackFrame_Security;

			case CorDebugInternalFrameType.STUBFRAME_JIT_COMPILATION:
				return NetSpy_Debugger_DotNet_CorDebug_Resources.StackFrame_JITCompilation;
			...
			default:
				return string.Format(NetSpy_Debugger_DotNet_CorDebug_Resources.StackFrame_InternalFrame, "0x" + ((int)corFrame.InternalFrameType).ToString("X8"));
			}
		}
