...
namespace NetSpy.Debugger.DotNet.CorDebug.Impl {
	abstract partial class DbgEngineImpl {
		...
		void SetIP_CorDebug(DbgThread thread, DbgCodeLocation location) {
			...
			if (failed) {
				SendMessage(new DbgMessageSetIPComplete(thread, framesInvalidated, messageFlags: GetMessageFlags(), error: NetSpy_Debugger_DotNet_CorDebug_Resources.Error_CouldNotSetNextStatement));
				return;
			}
		...
		string? TryGetFrameForSetIP_CorDebug(DbgThread thread, DbgCodeLocation location, out CorFrame? corFrame, out uint offset) {
			...
			if (dnDebugger.ProcessState != DebuggerProcessState.Paused)
				return NetSpy_Debugger_DotNet_CorDebug_Resources.Error_CouldNotSetNextStatement;
