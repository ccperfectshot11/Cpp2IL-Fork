...
using NetSpy.Contracts.Debugger.DotNet.Metadata.Internal;
...
using NetSpy.Debugger.DotNet.CorDebug.CallStack;
using NetSpy.Debugger.DotNet.CorDebug.Code;
using NetSpy.Debugger.DotNet.CorDebug.DAC;
using NetSpy.Debugger.DotNet.CorDebug.Impl.Attach;
using NetSpy.Debugger.DotNet.CorDebug.Impl.Evaluation;
using NetSpy.Debugger.DotNet.CorDebug.Properties;
using NetSpy.Debugger.DotNet.CorDebug.Steppers;
	...
	abstract partial class DbgEngineImpl : DbgEngine, IClrDacDebugger {
		...
		void DnDebugger_DebugCallbackEvent(DnDebugger dbg, DebugCallbackEventArgs e) {
			...
			case DebugCallbackKind.Exception2:
				...
				objectFactory.CreateException(new DbgExceptionId(PredefinedExceptionCategories.DotNet, exName ?? "???"), exFlags, exMessage ?? NetSpy_Debugger_DotNet_CorDebug_Resources.ExceptionMessageIsNull, hResult, TryGetThread(e2.CorThread), module, GetMessageFlags());
				e.AddPauseReason(DebuggerPauseReason.Other);
				break;
			...
			case DebugCallbackKind.DebuggerError:
				...
				if (deArgs.HError == CordbgErrors.CORDBG_E_UNCOMPATIBLE_PLATFORMS)
					...
					msg = string.Format(NetSpy_Debugger_DotNet_CorDebug_Resources.Error_CLRDebuggerErrorOccurred, deArgs.HError, deArgs.ErrorCode);
		...
		DbgModule? TryGetModule(CorFrame? frame, CorThread? thread) {
			if (frame?.Function is null && thread is not null) {
				...
				if (frame?.Function is null) {
					...
					// happens when debugging NetSpy built for .NET x86)
		...
		void StartCore(CorDebugStartDebuggingOptions options) {
			...
			try {
				...
				// Disable OS heap debugging https://github.com/NetSpy/NetSpy/issues/1106
				...
				if (ex is StartDebuggerException sde) {
					...
					case StartDebuggerError.UnsupportedBitness:
						errMsg = string.Format(NetSpy_Debugger_DotNet_CorDebug_Resources.Error_CouldNotStartDebugger, GetIncompatiblePlatformErrorMessage());
						break;
					default:
						throw new InvalidOperationException();
					}
					...
					if (IntPtr.Size == 8 && DotNetAssemblyUtilities.TryGetProgramBitness(options.Filename) == 32)
						...
						errMsg = NetSpy_Debugger_DotNet_CorDebug_Resources.Error_CouldNotStartDebuggerRequireAdminPrivLvl;
				}
				else {
					string exMessage = ex.Message;
					if (cex is not null)
						exMessage += $" (0x{cex.ErrorCode:X8})";
					errMsg = string.Format(NetSpy_Debugger_DotNet_CorDebug_Resources.Error_CouldNotStartDebuggerCheckAccessToFile, options.Filename ?? "<???>", exMessage);
				}
		...
		static string GetIncompatiblePlatformErrorMessage() {
			if (IntPtr.Size == 4)
				return NetSpy_Debugger_DotNet_CorDebug_Resources.UseNetSpyExeToDebug64;
			return NetSpy_Debugger_DotNet_CorDebug_Resources.UseNetSpy64ExeToDebug32;
		}
		...
		void AttachCore(CorDebugAttachToProgramOptions options) {
			...
			try {
				...
				if (dnDebugger.Processes.Length == 0)
					throw new ErrorException(string.Format(NetSpy_Debugger_DotNet_CorDebug_Resources.Error_CouldNotAttachToProcess, $"PID={options.ProcessId.ToString()}"));
				OnDebugProcess(dnDebugger);
				HookDnDebuggerEvents();
				return;
			}
				...
				if (ex is ErrorException errEx)
					...
					errMsg = string.Format(NetSpy_Debugger_DotNet_CorDebug_Resources.Error_CouldNotStartDebugger2,
						string.Format(NetSpy_Debugger_DotNet_CorDebug_Resources.Error_ProcessIsAlreadyBeingDebugged, options.ProcessId.ToString()));
				}
				else
					errMsg = string.Format(NetSpy_Debugger_DotNet_CorDebug_Resources.Error_CouldNotStartDebugger2, ex.Message);
			...
			if (dnDebugger.ProcessState == DebuggerProcessState.Running) {
				...
				if (hr < 0) {
					...
					if (hr == CordbgErrors.CORDBG_E_PROCESS_TERMINATED)
						return;
					SendMessage(new DbgMessageBreak(string.Format(NetSpy_Debugger_DotNet_CorDebug_Resources.Error_CouldNotBreakProcess, hr), GetMessageFlags()));
				}
				else {
