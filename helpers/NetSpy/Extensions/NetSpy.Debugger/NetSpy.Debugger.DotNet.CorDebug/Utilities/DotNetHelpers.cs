...
using dnlib.DotNet;
using dnlib.PE;
using NetSpy.Debugger.DotNet.CorDebug.Native;
using NetSpy.Debugger.Shared;
using Microsoft.Win32;

namespace NetSpy.Debugger.DotNet.CorDebug.Utilities {
	static class DotNetHelpers {
		...
		// NOTE: This same method exists in DotNetPathProvider (NetSpy project). Update both methods if this one gets updated.
		...
		public static string GetDebugShimFilename(int bitness) {
			...
			// If NetSpy is published, the file will be located in the app directory
			if (File.Exists(path))
				return path;
			// If NetSpy is ran from source, the file will be located in a different directory
