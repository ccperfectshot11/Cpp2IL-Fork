			...
			try {
				...
				for (;;) {
					...
					if (res == NativeMethods.WAIT_TIMEOUT) {
						if (keepWaiting())
							continue;
						throw new TimeoutException("Waiting for CoreCLR timed out. Debug 32-bit .NET apps with 32-bit NetSpy (NetSpy-x86.exe), and 64-bit .NET apps with 64-bit NetSpy (NetSpy.exe).");
					}
