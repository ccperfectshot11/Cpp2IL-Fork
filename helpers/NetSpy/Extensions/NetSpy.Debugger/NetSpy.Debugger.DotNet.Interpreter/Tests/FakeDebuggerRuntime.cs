...
namespace NetSpy.Debugger.DotNet.Interpreter.Tests.Fake {
	...
	sealed class FakeDebuggerRuntime : DebuggerRuntime {
		...
		sealed class FakeValueType : TypeILValue {
			...
			public override bool Call(bool isCallvirt, DmdMethodBase method, ILValue[] arguments, out ILValue? returnValue) {
				...
				else if (type.FullName == "NetSpy.Debugger.DotNet.Interpreter.Tests.MyStruct") {
					switch (method.Name) {
		...
		sealed class FakeReferenceType : TypeILValue {
			...
			public override bool Call(bool isCallvirt, DmdMethodBase method, ILValue[] arguments, out ILValue? returnValue) {
				var ad = method.AppDomain;
				var type = method.ReflectedType;
				if (type.FullName == "NetSpy.Debugger.DotNet.Interpreter.Tests.MyClass") {
					switch (method.Name) {
					case "InstanceMethod1":
						returnValue = new ConstantInt32ILValue(ad, 123);
						return true;
		...
		public override bool CallStatic(DmdMethodBase method, ILValue[] arguments, out ILValue? returnValue) {
			var ad = method.AppDomain;
			var type = method.ReflectedType;
			if (type.FullName == "NetSpy.Debugger.DotNet.Interpreter.Tests.MyStruct") {
				switch (method.Name) {
				case "StaticMethod1":
					returnValue = new ConstantInt32ILValue(ad, 123);
					return true;
			...
			else if (type.FullName == "NetSpy.Debugger.DotNet.Interpreter.Tests.MyClass") {
				switch (method.Name) {
				case "StaticMethod1":
					returnValue = new ConstantInt32ILValue(ad, 123);
					return true;
