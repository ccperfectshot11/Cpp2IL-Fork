...
namespace NetSpy.Debugger.DotNet.Interpreter.Tests {
	...
	public sealed class InterpreterTest {
		string GetTestAssemblyFilename() => Path.GetFullPath(Path.GetDirectoryName(GetType().Assembly.Location) + @"\..\..\..\NetSpy.Console\bin\NetSpy.Debugger.DotNet.Interpreter.Tests.dll");
		...
		void TestCore2() {
			testRuntime = CreateTestRuntime();
			testAsm1 = testRuntime.Runtime.GetAppDomains().First().GetAssembly("NetSpy.Debugger.DotNet.Interpreter.Tests") ?? throw new InvalidOperationException();
			testAsm2 = Assembly.LoadFile(testAsm1.Location);
			testType1 = testAsm1.GetType("NetSpy.Debugger.DotNet.Interpreter.Tests.TestClass") ?? throw new InvalidOperationException();
