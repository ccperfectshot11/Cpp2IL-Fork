...
using NetSpy.Debugger.DotNet.Mono.Impl;
using NetSpy.Debugger.DotNet.Mono.Properties;
using Mono.Debugger.Soft;
using MDS = Mono.Debugger.Soft;

namespace NetSpy.Debugger.DotNet.Mono.CallStack {
		...
		DbgEngineStackFrame CreateEngineStackFrame(MDS.StackFrame monoFrame, int frameIndex) {
			...
			if (method is null) {
				if (monoFrame.IsDebuggerInvoke)
					return engine.ObjectFactory.CreateSpecialStackFrame(NetSpy_Debugger_DotNet_Mono_Resources.StackFrame_FunctionEvaluation);
				if (monoFrame.IsNativeTransition)
					return engine.ObjectFactory.CreateSpecialStackFrame(NetSpy_Debugger_DotNet_Mono_Resources.StackFrame_NativeTransition);
