...
namespace NetSpy.Debugger.DotNet.Mono.Dialogs.AttachToProcess {
		...
		public override IEnumerable<AttachProgramOptions> Create(AttachProgramOptionsProviderContext context) {
			...
			try {
				foreach (var p in processes) {
					...
					yield return new UnityAttachProgramOptionsImpl(p.Id, ipAddress, port, $"Unity ({NetSpy_Debugger_DotNet_Mono_Resources.UnityEditor})");
				}
			}
			finally {
				foreach (var p in processes)
