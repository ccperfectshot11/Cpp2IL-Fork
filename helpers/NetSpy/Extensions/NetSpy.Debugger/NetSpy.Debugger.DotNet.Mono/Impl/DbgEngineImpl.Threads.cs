	...
	sealed partial class DbgEngineImpl {
		...
		void SetIP_MonoDebug(DbgThread thread, DbgCodeLocation location) {
			...
			if (!vm!.Version.AtLeast(2, 29)) {
				SendMessage(new DbgMessageSetIPComplete(thread, false, NetSpy_Debugger_DotNet_Mono_Resources.Error_RuntimeDoesNotSupportSettingNewStatement, GetMessageFlags()));
				return;
			}
			var threadData = TryGetThreadData(thread);
			if (isUnhandledException || threadData is null || !(location is IDbgDotNetCodeLocation loc)) {
				SendMessage(new DbgMessageSetIPComplete(thread, false, NetSpy_Debugger_DotNet_Mono_Resources.Error_CouldNotSetNextStatement, GetMessageFlags()));
				return;
			}
			...
			try {
				...
				SendMessage(new DbgMessageSetIPComplete(thread, true, NetSpy_Debugger_DotNet_Mono_Resources.Error_CouldNotSetNextStatement, GetMessageFlags()));
			}
		}
		...
		string? SetIPCore_MonoDebug(DbgThreadData threadData, IDbgDotNetCodeLocation location, out bool framesInvalidated) {
			...
			if (!VerifySetIPLocation(threadData, location, out var monoMethod, out var frame))
				return NetSpy_Debugger_DotNet_Mono_Resources.Error_CouldNotSetNextStatement;
