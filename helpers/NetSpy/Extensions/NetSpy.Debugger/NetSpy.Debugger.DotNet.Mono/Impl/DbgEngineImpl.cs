...
using NetSpy.Debugger.DotNet.Mono.CallStack;
using NetSpy.Debugger.DotNet.Mono.Impl.Attach;
using NetSpy.Debugger.DotNet.Mono.Impl.Evaluation;
...
namespace NetSpy.Debugger.DotNet.Mono.Impl {
	sealed partial class DbgEngineImpl : DbgEngine {
			...
			try {
				...
				if (options is MonoStartDebuggingOptions startMonoOptions) {
					...
					if (!File.Exists(monoExe))
						throw new StartException(string.Format(NetSpy_Debugger_DotNet_Mono_Resources.Error_CouldNotFindFile, MonoExeFinder.MONO_EXE));
					...
					const string ENV_VAR_NAME_V0 = "NETSPY_UNITY_DBG";
					const string ENV_VAR_NAME_V1 = "NETSPY_UNITY_DBG2";
					string envVarValue;
					Debug.Assert(!connectionAddress.Contains(" "));
					...
					defaultCouldNotConnectMessage = GetCouldNotConnectErrorMessage(connectionAddress, connectionPort, filename) + "\r\n\r\n" +
						NetSpy_Debugger_DotNet_Mono_Resources.CouldNotConnectToUnityGame_MakeSureMonoDllFileIsPatched;
				}
				...
				if (pid is null)
					throw new StartException(NetSpy_Debugger_DotNet_Mono_Resources.Error_CouldNotFindDebuggedProcess);
				vmPid = pid.Value;

				hProcess_debuggee = NativeMethods.OpenProcess(NativeMethods.PROCESS_QUERY_LIMITED_INFORMATION, false, (uint)vmPid);

				dbgManager.WriteMessage(string.Format(NetSpy_Debugger_DotNet_Mono_Resources.MonoDebuggerConnectionMessage, ep, vm.Version.VMVersion, $"{vm.Version.MajorVersion}.{vm.Version.MinorVersion}"));
				...
				if (ex is CouldNotConnectException)
					...
					msg = NetSpy_Debugger_DotNet_Mono_Resources.Error_CouldNotConnectToProcess;
				else if (ex is StartException)
					msg = ex.Message;
				else {
					msg = defaultCouldNotConnectMessage ??
						NetSpy_Debugger_DotNet_Mono_Resources.Error_CouldNotConnectToProcess + "\r\n\r\n" + ex.Message;
				}
			...
			for (int i = 0; i < eventSet.Events.Length; i++) {
				...
				case EventType.Crash:
					...
					if (vmDeathExitCode is null) {
						vmDeathExitCode = -1;
						dbgManager.ShowError(NetSpy_Debugger_DotNet_Mono_Resources.Error_ConnectionWasUnexpectedlyClosed);
					}
		...
		static string GetCouldNotConnectErrorMessage(string address, ushort port, string? filenameOpt) {
			...
			return NetSpy_Debugger_DotNet_Mono_Resources.Error_CouldNotConnectToProcess + extra;
		}
