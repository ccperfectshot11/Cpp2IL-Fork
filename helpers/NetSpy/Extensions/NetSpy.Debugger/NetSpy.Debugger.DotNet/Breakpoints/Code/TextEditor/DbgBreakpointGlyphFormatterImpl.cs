...
using NetSpy.Debugger.DotNet.Properties;
using Microsoft.VisualStudio.Text;
using Microsoft.VisualStudio.Text.Editor;

namespace NetSpy.Debugger.DotNet.Breakpoints.Code.TextEditor {
	...
	sealed class DbgBreakpointGlyphFormatterImpl : DbgBreakpointGlyphFormatter {
		...
		bool WriteLocation(IDbgTextWriter output, ITextView textView, SnapshotSpan span, DbgDotNetCodeLocation location) {
			...
			output.Write(DbgTextColor.Text, string.Format(NetSpy_Debugger_DotNet_Resources.GlyphToolTip_line_0_character_1,
			...
			case DbgILOffsetMapping.Approximate:
				...
				output.Write(DbgTextColor.Text, string.Format(NetSpy_Debugger_DotNet_Resources.GlyphToolTip_IL_offset_0, prefix + location.Offset.ToString("X4")));
				break;

			case DbgILOffsetMapping.Prolog:
				output.Write(DbgTextColor.Text, string.Format(NetSpy_Debugger_DotNet_Resources.GlyphToolTip_IL_offset_0, "(prolog)"));
				break;

			case DbgILOffsetMapping.Epilog:
				output.Write(DbgTextColor.Text, string.Format(NetSpy_Debugger_DotNet_Resources.GlyphToolTip_IL_offset_0, "(epilog)"));
				break;

			case DbgILOffsetMapping.Unknown:
			case DbgILOffsetMapping.NoInfo:
			case DbgILOffsetMapping.UnmappedAddress:
				output.Write(DbgTextColor.Text, string.Format(NetSpy_Debugger_DotNet_Resources.GlyphToolTip_IL_offset_0, "(???)"));
				break;
