...
using NetSpy.Contracts.Debugger.DotNet.Evaluation.ExpressionCompiler;
...
namespace NetSpy.Debugger.DotNet.Evaluation.Engine.Interpreter {
	...
	sealed class DebuggerRuntimeImpl : DebuggerRuntime2, IDebuggerRuntime {
		...
		DbgDotNetValue IDebuggerRuntime.GetException() {
			...
			if (value is null)
				throw new InterpreterMessageException(NetSpy_Debugger_DotNet_Resources.NoExceptionOnTheCurrentThread);
			return value;
		}
		...
		DbgDotNetValue IDebuggerRuntime.GetReturnValue(int index) {
			...
			if (value is null)
				throw new InterpreterMessageException(NetSpy_Debugger_DotNet_Resources.ReturnValueNotAvailable);
			return value;
		}
		...
		DbgDotNetValue IDebuggerRuntime.GetObjectByAlias(string name) {
			...
			throw new InterpreterMessageException(NetSpy_Debugger_DotNet_Resources.UnknownVariableOrObjectId);
		}
		...
		DbgDotNetValue IDebuggerRuntime.GetObjectAtAddress(ulong address) {
			...
			if (value is null)
				throw new InterpreterMessageException(NetSpy_Debugger_DotNet_Resources.NoDotNetObjectFoundAtAddress);
			return value;
		}
