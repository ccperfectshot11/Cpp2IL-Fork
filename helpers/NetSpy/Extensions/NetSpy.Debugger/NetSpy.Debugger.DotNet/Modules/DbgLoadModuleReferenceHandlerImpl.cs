...
using NetSpy.Contracts.Debugger.References;
using NetSpy.Contracts.Documents;
using NetSpy.Contracts.Documents.Tabs;
using NetSpy.Debugger.DotNet.Properties;
using NetSpy.Debugger.DotNet.UI;

namespace NetSpy.Debugger.DotNet.Modules {
	...
	sealed class DbgLoadModuleReferenceHandlerImpl : DbgLoadModuleReferenceHandler {
		...
		bool GoToCore(DbgLoadModuleReference moduleRef, ReadOnlyCollection<object> options) {
			...
			if (md is null) {
				if (canShowMessageBox)
					messageBoxService.Value.Show(NetSpy_Debugger_DotNet_Resources.FailedToObtainValidInMemoryMetadataForModule);
				return false;
			}
		...
		ModuleDef? LoadModule(DbgModule module, DbgLoadModuleOptions options, ref bool canShowMessageBox) {
			...
			if (module.IsDynamic && !module.Runtime.IsClosed && module.Process.IsRunning) {
				if (canShowMessageBox) {
					canShowMessageBox = false;
					messageBoxService.Value.Show(NetSpy_Debugger_DotNet_Resources.Module_BreakProcessBeforeLoadingDynamicModules);
				}
				return null;
			}
