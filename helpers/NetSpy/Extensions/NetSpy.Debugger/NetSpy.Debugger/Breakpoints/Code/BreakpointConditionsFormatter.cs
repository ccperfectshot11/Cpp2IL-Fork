...
using NetSpy.Debugger.Breakpoints.Code.CondChecker;
using NetSpy.Debugger.Properties;

namespace NetSpy.Debugger.Breakpoints.Code {
	...
	sealed class BreakpointConditionsFormatterImpl : BreakpointConditionsFormatter {
		...
		public override void Write(IDbgTextWriter output, DbgCodeBreakpointCondition? condition) {
			...
			if (condition is null)
				output.Write(DbgTextColor.Text, NetSpy_Debugger_Resources.Breakpoint_Condition_NoCondition);
			else {
				switch (condition.Value.Kind) {
				case DbgCodeBreakpointConditionKind.IsTrue:
					WriteArgumentAndText(output, DbgTextColor.String, NetSpy_Debugger_Resources.Breakpoint_Condition_WhenConditionIsTrue, condition.Value.Condition);
					break;

				case DbgCodeBreakpointConditionKind.WhenChanged:
					WriteArgumentAndText(output, DbgTextColor.String, NetSpy_Debugger_Resources.Breakpoint_Condition_WhenConditionHasChanged, condition.Value.Condition);
					break;
		...
		public override void Write(IDbgTextWriter output, DbgCodeBreakpointHitCount? hitCount, int? currentHitCount) {
			if (output is null)
				throw new ArgumentNullException(nameof(output));
			if (hitCount is null)
				output.Write(DbgTextColor.Text, NetSpy_Debugger_Resources.Breakpoint_HitCount_NoHitCount);
			else {
				switch (hitCount.Value.Kind) {
				case DbgCodeBreakpointHitCountKind.Equals:
					WriteArgumentAndText(output, DbgTextColor.Number, NetSpy_Debugger_Resources.Breakpoint_HitCount_HitCountIsEqualTo, hitCount.Value.Count.ToString());
					break;

				case DbgCodeBreakpointHitCountKind.MultipleOf:
					WriteArgumentAndText(output, DbgTextColor.Number, NetSpy_Debugger_Resources.Breakpoint_HitCount_HitCountIsAMultipleOf, hitCount.Value.Count.ToString());
					break;

				case DbgCodeBreakpointHitCountKind.GreaterThanOrEquals:
					WriteArgumentAndText(output, DbgTextColor.Number, NetSpy_Debugger_Resources.Breakpoint_HitCount_HitCountIsGreaterThanOrEqualTo, hitCount.Value.Count.ToString());
					break;
		...
		void WriteCurrentHitCountValue(IDbgTextWriter output, int? currentHitCount) {
			if (currentHitCount is not null) {
				...
				WriteArgumentAndText(output, DbgTextColor.Number, NetSpy_Debugger_Resources.Breakpoint_HitCount_CurrentHitCountValue, currentHitCount.Value.ToString());
				output.Write(DbgTextColor.Punctuation, ")");
			}
		}
		...
		public override void Write(IDbgTextWriter output, DbgCodeBreakpointFilter? filter) {
			if (output is null)
				throw new ArgumentNullException(nameof(output));
			if (filter is null)
				output.Write(DbgTextColor.Text, NetSpy_Debugger_Resources.Breakpoint_Filter_NoFilter);
			else
		...
		public override void Write(IDbgTextWriter output, DbgCodeBreakpointTrace? trace) {
			if (output is null)
				throw new ArgumentNullException(nameof(output));
			if (trace is null)
				output.Write(DbgTextColor.Text, NetSpy_Debugger_Resources.Breakpoint_Tracepoint_NoTraceMessage);
			else {
				var traceTmp = trace.Value;
				if (traceTmp.Continue)
					WriteArgumentAndText(output, DbgTextColor.Comment, NetSpy_Debugger_Resources.Breakpoint_Tracepoint_PrintMessage, () => tracepointMessageCreatorImpl.Value.Write(output, traceTmp));
				else
					WriteArgumentAndText(output, DbgTextColor.Comment, NetSpy_Debugger_Resources.Breakpoint_Tracepoint_BreakPrintMessage, () => tracepointMessageCreatorImpl.Value.Write(output, traceTmp));
			}
		}
		...
		public override void WriteToolTip(IDbgTextWriter output, DbgCodeBreakpointCondition condition) {
			...
			output.Write(defaultColor, NetSpy_Debugger_Resources.Breakpoint_Condition_ConditionalExpression);
			output.Write(DbgTextColor.Text, " ");
			switch (condition.Kind) {
			case DbgCodeBreakpointConditionKind.IsTrue:
				WriteArgumentAndText(output, defaultColor, DbgTextColor.String, NetSpy_Debugger_Resources.Breakpoint_Condition_WhenConditionIsTrue2, condition.Condition);
				break;

			case DbgCodeBreakpointConditionKind.WhenChanged:
				WriteArgumentAndText(output, defaultColor, DbgTextColor.String, NetSpy_Debugger_Resources.Breakpoint_Condition_WhenConditionHasChanged2, condition.Condition);
				break;
		...
		public override void WriteToolTip(IDbgTextWriter output, DbgCodeBreakpointHitCount hitCount, int? currentHitCount) {
			...
			case DbgCodeBreakpointHitCountKind.Equals:
				WriteArgumentAndText(output, defaultColor, DbgTextColor.Number, NetSpy_Debugger_Resources.Breakpoint_HitCount_HitCountIsEqualTo2, hitCount.Count.ToString());
				break;

			case DbgCodeBreakpointHitCountKind.MultipleOf:
				WriteArgumentAndText(output, defaultColor, DbgTextColor.Number, NetSpy_Debugger_Resources.Breakpoint_HitCount_HitCountIsAMultipleOf2, hitCount.Count.ToString());
				break;

			case DbgCodeBreakpointHitCountKind.GreaterThanOrEquals:
				WriteArgumentAndText(output, defaultColor, DbgTextColor.Number, NetSpy_Debugger_Resources.Breakpoint_HitCount_HitCountIsGreaterThanOrEqualTo2, hitCount.Count.ToString());
				break;
		...
		public override void WriteToolTip(IDbgTextWriter output, DbgCodeBreakpointFilter filter) {
			...
			WriteArgumentAndText(output, defaultColor, NetSpy_Debugger_Resources.Breakpoint_Filter_Filter, () => dbgFilterExpressionEvaluatorService.Value.Write(output, filterTmp.Filter ?? string.Empty));
		}
		...
		public override void WriteToolTip(IDbgTextWriter output, DbgCodeBreakpointTrace trace) {
			...
			var defaultColor = DbgTextColor.Text;
			var traceTmp = trace;
			WriteArgumentAndText(output, defaultColor, NetSpy_Debugger_Resources.Breakpoint_Tracepoint_PrintMessage2, () => tracepointMessageCreatorImpl.Value.Write(output, traceTmp));
		}
	}
}
