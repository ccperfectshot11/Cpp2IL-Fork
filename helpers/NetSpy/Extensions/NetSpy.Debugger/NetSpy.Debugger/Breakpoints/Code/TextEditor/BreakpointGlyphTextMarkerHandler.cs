...
using NetSpy.Contracts.Debugger.Breakpoints.Code.Dialogs;
...
namespace NetSpy.Debugger.Breakpoints.Code.TextEditor {
	...
	sealed class BreakpointGlyphTextMarkerHandlerImpl : BreakpointGlyphTextMarkerHandler {
		...
		string GetToolTipContent(DbgCodeBreakpoint breakpoint, ITextView textView, SnapshotSpan span) {
			...
			output.Write(DbgTextColor.Text, NetSpy_Debugger_Resources.GlyphToolTip_Location);
			...
			if (breakpoint.Condition is not null || breakpoint.HitCount is not null || breakpoint.Filter is not null) {
				output.WriteLine();
				output.WriteLine();
				output.Write(DbgTextColor.Text, NetSpy_Debugger_Resources.GlyphToolTip_Conditions);
			...
			if (breakpoint.Trace is not null) {
				output.WriteLine();
				output.WriteLine();
				output.Write(DbgTextColor.Text, NetSpy_Debugger_Resources.GlyphToolTip_Actions);

				if (!breakpoint.Trace.Value.Continue) {
					output.WriteLine();
					output.Write(DbgTextColor.Text, INDENTATION);
					output.Write(DbgTextColor.Text, NetSpy_Debugger_Resources.Breakpoints_GlyphMargin_BreakWhenBreakpointHit);
				}
