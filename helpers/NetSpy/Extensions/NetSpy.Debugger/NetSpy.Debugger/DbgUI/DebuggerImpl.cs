...
using NetSpy.Contracts.Debugger.Steppers;
using NetSpy.Contracts.Documents;
using NetSpy.Contracts.Documents.Tabs;
using NetSpy.Debugger.Breakpoints.Code.TextEditor;
using NetSpy.Debugger.Code.TextEditor;
using NetSpy.Debugger.Disassembly;
...
namespace NetSpy.Debugger.DbgUI {
	...
	sealed class DebuggerImpl : Debugger, IDbgManagerStartListener {
		...
		void AppWindow_MainWindowClosing(object? sender, CancelEventArgs e) {
			if (IsDebugging) {
				var result = messageBoxService.Value.ShowIgnorableMessage(new Guid("B4B8E13C-B7B7-490A-953B-8ED8EAE7C170"), NetSpy_Debugger_Resources.AskAppWindowClosingStopDebugging, MsgBoxButton.Yes | MsgBoxButton.No);
		...
		public override void StartWithoutDebugging() {
			...
			if ((result & StartDebuggingResult.WrongExtension) != 0) {
				if (messageBoxService.Value.Show(NetSpy_Debugger_Resources.RunWithInvalidExtension, MsgBoxButton.Yes | MsgBoxButton.No) != MsgBoxButton.Yes)
					return;
			}
		...
		public override void DebugProgram(bool pauseAtEntryPoint) {
			...
			if ((flags & StartDebuggingOptionsInfoFlags.WrongExtension) != 0) {
				if (messageBoxService.Value.Show(NetSpy_Debugger_Resources.DebugWithInvalidExtension, MsgBoxButton.Yes | MsgBoxButton.No) != MsgBoxButton.Yes)
					return;
			}
		...
		public override void DetachAll() {
			if (!dbgManager.Value.CanDetachWithoutTerminating && messageBoxService.Value.Show(NetSpy_Debugger_Resources.DetachingWillCauseTerminationOfDebuggeeProcess, MsgBoxButton.Yes | MsgBoxButton.No) != MsgBoxButton.Yes)
				return;
		...
		public override void DeleteAllBreakpointsAskUser() {
			var res = messageBoxService.Value.ShowIgnorableMessage(new Guid("37250D26-E844-49F4-904B-29600B90476C"), NetSpy_Debugger_Resources.AskDeleteAllBreakpoints, MsgBoxButton.Yes | MsgBoxButton.No);
		...
		void ShowUnhandledException_UI(DbgMessageExceptionThrownEventArgs exm) {
			...
			sb.AppendLine(string.Format(NetSpy_Debugger_Resources.UnhandledExceptionMessage_ProcessName_ProcessId, exm.Exception.Process.Name, exm.Exception.Process.Id));
			sb.AppendLine();
			sb.AppendLine(string.Format(NetSpy_Debugger_Resources.ExceptionName, dbgExceptionFormatterService.Value.ToString(exm.Exception.Id)));
			sb.AppendLine();
			sb.AppendLine(string.Format(NetSpy_Debugger_Resources.ExceptionMessage, exm.Exception.Message ?? NetSpy_Debugger_Resources.ExceptionMessageIsNull));
			if (exm.Exception.HResult.HasValue) {
				sb.AppendLine();
				sb.AppendLine(string.Format(NetSpy_Debugger_Resources.ExceptionHResult, exm.Exception.HResult.Value));
			}
		...
		void DbgManager_IsDebuggingChanged(object? sender, EventArgs e) {
			...
			UI(() => {
				...
				if (newIsDebugging) {
					...
					appWindow.Value.AddTitleInfo(NetSpy_Debugger_Resources.AppTitle_Debugging);
				}
				else {
					appWindow.Value.StatusBar.Close();
					appWindow.Value.RemoveTitleInfo(NetSpy_Debugger_Resources.AppTitle_Debugging);
				}
		...
		void SetRunningStatusMessage_UI() => SetStatusMessage_UI(NetSpy_Debugger_Resources.StatusBar_Running);
		...
		static string GetStatusBarMessage(IList<DbgBreakInfo> breakInfos) {
			if (breakInfos.Count == 0)
				return NetSpy_Debugger_Resources.StatusBar_Running;
			...
			case DbgBreakInfoKind.Message:
				...
				case DbgMessageKind.ModuleLoaded:
					...
					if (module.IsDynamic || module.IsInMemory)
						return string.Format(NetSpy_Debugger_Resources.Debug_EventDescription_LoadModule1, module.IsDynamic ? 1 : 0, module.IsInMemory ? 1 : 0, module.Address, module.Size, module.Name);
					return string.Format(NetSpy_Debugger_Resources.Debug_EventDescription_LoadModule2, module.Address, module.Size, module.Name);

				case DbgMessageKind.ExceptionThrown:
					var ex = ((DbgMessageExceptionThrownEventArgs)e).Exception;
					var exMsg = ex.IsUnhandled ? NetSpy_Debugger_Resources.Debug_EventDescription_UnhandledException : NetSpy_Debugger_Resources.Debug_EventDescription_Exception;
				...
				case DbgMessageKind.BoundBreakpoint:
					...
					var bpMsg = $"{NetSpy_Debugger_Resources.StatusBar_BreakpointHit} #{bbe.BoundBreakpoint.Breakpoint.Id} : pid={bbe.BoundBreakpoint.Process.Id}({GetProcessName(bbe.BoundBreakpoint.Process)})";
			...
			return NetSpy_Debugger_Resources.StatusBar_Ready;
		}
