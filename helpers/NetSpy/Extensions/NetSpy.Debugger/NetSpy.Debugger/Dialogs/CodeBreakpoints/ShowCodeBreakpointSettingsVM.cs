...
namespace NetSpy.Debugger.Dialogs.CodeBreakpoints {
	sealed class ShowCodeBreakpointSettingsVM : ViewModelBase, IDisposable {
		...
		static readonly EnumVM[] conditionList = new EnumVM[] {
			new EnumVM(DbgCodeBreakpointConditionKind.IsTrue, NetSpy_Debugger_Resources.Breakpoint_Condition_IsTrue),
			new EnumVM(DbgCodeBreakpointConditionKind.WhenChanged, NetSpy_Debugger_Resources.Breakpoint_Condition_WhenChanged),
		};
		...
		static readonly EnumVM[] hitCountList = new EnumVM[] {
			...
			new EnumVM(DbgCodeBreakpointHitCountKind.MultipleOf, NetSpy_Debugger_Resources.Breakpoint_HitCount_IsAMultipleOf),
		...
		void ShowFilterExpressionHelp() {
			...
			sb.AppendLine(string.Format(NetSpy_Debugger_Resources.FilterExpression_Example, @"ProcessId == 0x1234 && ThreadName == ""Main Thread"""));
			sb.AppendLine();
			sb.AppendLine(NetSpy_Debugger_Resources.FilterExpression_AllVariables);
			sb.Append(@"
MachineName
ProcessId
ProcessName
...
ThreadName");
		...
		void ShowTracepointMessageHelp() {
			var sb = new StringBuilder();
			sb.AppendLine(string.Format(NetSpy_Debugger_Resources.TracepointMessage_Example, @"$CALLERTOKEN: string: {s}"));
			sb.AppendLine();
			sb.AppendLine(NetSpy_Debugger_Resources.TracepointMessage_EvaluateMessage);
			sb.AppendLine();
			sb.AppendLine(string.Format(NetSpy_Debugger_Resources.TracepointMessage_EscapeSequences, @"\\ \$ \{ \} \a \b \f \n \r \t \v"));
			sb.AppendLine();
			sb.AppendLine(NetSpy_Debugger_Resources.TracepointMessage_ValidKeywords);
			sb.Append(@"
