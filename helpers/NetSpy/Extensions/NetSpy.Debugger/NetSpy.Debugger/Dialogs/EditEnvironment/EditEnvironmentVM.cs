...
namespace NetSpy.Debugger.Dialogs.EditEnvironment {
	sealed class EditEnvironmentVM : ViewModelBase {
		...
		void AddManyEnvironmentVariables() {
			var variables = MsgBox.Instance.Ask(NetSpy_Debugger_Resources.AddEnvironmentVariablesMsgBoxLabel, null, NetSpy_Debugger_Resources.AddEnvironmentVariablesMsgBoxTitle, s => {
				new EnvironmentStringParser(s).TryParse(out var env);
				return env;
			}, s => new EnvironmentStringParser(s).TryParse(out _) ? null : NetSpy_Debugger_Resources.InvalidInputString);
