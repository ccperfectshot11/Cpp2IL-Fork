...
namespace NetSpy.Debugger.Evaluation.ViewModel.Impl {
	...
	sealed class ValueNodeImpl : ValueNode, INotifyPropertyChanged {
		...
		public override string RefreshExpressionToolTip => NetSpy_Debugger_Resources.RefreshExpressionButtonToolTip;
		...
		public override IEnumerable<TreeNodeData> CreateChildren() {
			...
			if (childCount > MAX_CHILDREN) {
				bool open = Context.ShowMessageBox(string.Format(NetSpy_Debugger_Resources.Locals_Ask_TooManyItems, MAX_CHILDREN), ShowMessageBoxButtons.YesNo);
				if (!open) {
					ResetLazyLoading();
					yield break;
				}
