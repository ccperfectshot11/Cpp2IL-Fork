	...
	sealed class CodeBreakpointsOperationsImpl : CodeBreakpointsOperations {
		...
		public override void GoToDisassembly() {
			...
			if (!dbgShowNativeCodeService.Value.ShowNativeCode(boundBps[0]))
				messageBoxService.Show(NetSpy_Debugger_Resources.Error_CouldNotShowDisassembly);
		}
		...
		public override void EditLabels() {
			...
			if (SelectedItems.Count == 1)
				...
				var newLabels = messageBoxService.Ask<string>(NetSpy_Debugger_Resources.EditLabelsMsgBoxLabel, SelectedItems[0].GetLabelsString(), NetSpy_Debugger_Resources.EditLabelsTitle);
