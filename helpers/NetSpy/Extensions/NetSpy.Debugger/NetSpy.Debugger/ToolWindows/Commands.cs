...
namespace NetSpy.Debugger.ToolWindows {
	...
	sealed class SubMenuWatchWindowCommand : MenuItemBase, IMenuItemProvider {
		...
		static string GetInputGestureText(int i) {
			if (0 <= i && i <= 9)
				return string.Format(NetSpy_Debugger_Resources.ShortCutKeyCtrlAltW_DIGIT, (i + 1) % 10);
			return string.Empty;
		}
	...
	sealed class SubMenuMemoryWindowCommand : MenuItemBase, IMenuItemProvider {
		...
		static string GetInputGestureText(int i) {
			if (i == 0)
				return NetSpy_Debugger_Resources.ShortCutKeyAlt6;
			if (1 <= i && i <= 9)
				return string.Format(NetSpy_Debugger_Resources.ShortCutKeyCtrlShift_DIGIT, (i + 1) % 10);
			return string.Empty;
		}
