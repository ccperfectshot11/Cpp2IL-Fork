		...
		public void WriteConditions(IDbgTextWriter output, ExceptionVM vm) {
			...
			for (int i = 0; i < conditions.Count; i++) {
				if (i != 0) {
					...
					output.Write(DbgTextColor.Keyword, NetSpy_Debugger_Resources.Exception_Conditions_And);
					output.Write(DbgTextColor.Text, " ");
				}
				var cond = conditions[i];
				switch (cond.ConditionType) {
				case DbgExceptionConditionType.ModuleNameEquals:
					WriteQuotedString(output, NetSpy_Debugger_Resources.Exception_Conditions_ModuleNameEquals, cond.Condition);
					break;

				case DbgExceptionConditionType.ModuleNameNotEquals:
					WriteQuotedString(output, NetSpy_Debugger_Resources.Exception_Conditions_ModuleNameNotEquals, cond.Condition);
					break;
