...
namespace NetSpy.Debugger.ToolWindows {
	static class FormatterUtils {
		...
		public static T Write<T>(this T output, DbgAppDomain? appDomain) where T : IDbgTextWriter {
			if (appDomain is null)
				output.Write(DbgTextColor.Error, NetSpy_Debugger_Resources.AppDomainNotAvailable);
			else {
		...
		public static T WriteYesNo<T>(this T output, bool value) where T : IDbgTextWriter {
			if (value)
				output.Write(DbgTextColor.Text, NetSpy_Debugger_Resources.YesNo_Yes);
			else
				output.Write(DbgTextColor.Text, NetSpy_Debugger_Resources.YesNo_No);
			return output;
		}
	}
}
