...
namespace NetSpy.Debugger.ToolWindows.Logger {
	...
	sealed class OutputLogger : IOutputServiceListener2, IDbgManagerStartListener, ITracepointMessageListener {
		...
		void Initialize_UI() {
			...
			if (textPane is not null)
				return;
			textPane = outputService.Value.Create(GUID_OUTPUT_LOGGER_DEBUG, NetSpy_Debugger_Resources.DebugLoggerName, contentTypeRegistryService.Value.GetContentType(ContentTypes.OutputDebug));
		}
		...
		void DbgManager_MessageProcessExited(object? sender, DbgMessageProcessExitedEventArgs e) {
			if (outputLoggerSettings.ShowProcessExitMessages) {
				var msg = string.Format(NetSpy_Debugger_Resources.DebugLogExitProcess, GetProcessNameWithPID(e.Process), e.ExitCode);
		...
		void DbgManager_MessageModuleLoaded(object? sender, DbgMessageModuleLoadedEventArgs e) {
			if (outputLoggerSettings.ShowModuleLoadMessages) {
				...
				var msg = GetProcessName(module.Process) + " (" + GetRuntimeAppDomainName(module) + "): " +
						string.Format(NetSpy_Debugger_Resources.DebugLogLoadModule, module.Filename);
		...
		void DbgManager_MessageModuleUnloaded(object? sender, DbgMessageModuleUnloadedEventArgs e) {
			if (outputLoggerSettings.ShowModuleUnloadMessages) {
				var module = e.Module;
				var msg = GetProcessName(module.Process) + " (" + GetRuntimeAppDomainName(module) + "): " +
						string.Format(NetSpy_Debugger_Resources.DebugLogUnloadModule, module.Filename);
		...
		void DbgManager_MessageThreadExited(object? sender, DbgMessageThreadExitedEventArgs e) {
			if (outputLoggerSettings.ShowThreadExitMessages) {
				var msg = string.Format(NetSpy_Debugger_Resources.DebugLogExitThread, e.Thread.Id, e.ExitCode);
		...
		void DbgManager_MessageExceptionThrown(object? sender, DbgMessageExceptionThrownEventArgs e) {
			...
			if (ex.Id.Category == PredefinedExceptionCategories.MDA) {
				if (outputLoggerSettings.ShowMDAMessages) {
					...
					var msg1 = string.Format(NetSpy_Debugger_Resources.DebugLogMDA, GetExceptionName(ex.Id), GetProcessFullPath(process));
					var msg2 = ex.Message is null ? null : string.Format(NetSpy_Debugger_Resources.DebugLogAdditionalInformation, ex.Message);
					UI(() => {
				...
				if (outputLoggerSettings.ShowExceptionMessages) {
					if (ex.IsFirstChance) {
						var msg1 = string.Format(NetSpy_Debugger_Resources.DebugLogExceptionHandled, GetExceptionName(ex.Id), GetModuleName(ex.Module));
						var msg2 = ex.Message is null ? null : string.Format(NetSpy_Debugger_Resources.DebugLogAdditionalInformation, FilterUserMessage(ex.Message));
						UI(() => {
						...
						var msg1 = string.Format(NetSpy_Debugger_Resources.DebugLogExceptionUnhandled, GetExceptionName(ex.Id), GetModuleName(ex.Module));
						var msg2 = ex.Message is null ? null : string.Format(NetSpy_Debugger_Resources.DebugLogAdditionalInformation, FilterUserMessage(ex.Message));
						UI(() => {
