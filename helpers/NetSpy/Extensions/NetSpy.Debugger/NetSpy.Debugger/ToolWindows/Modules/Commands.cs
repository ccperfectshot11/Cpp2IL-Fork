...
using NetSpy.Debugger.ToolWindows.Memory;
...
namespace NetSpy.Debugger.ToolWindows.Modules {
	...
	sealed class LoadModulesModulesCtxMenuCommand : ModulesCtxMenuCommand {
		...
		public override string? GetHeader(ModulesCtxMenuContext context) {
			if (context.Operations.LoadModulesCount <= 1)
				return NetSpy_Debugger_Resources.LoadModulesCommand;
			return string.Format(NetSpy_Debugger_Resources.LoadXModulesCommand, context.Operations.LoadModulesCount);
		}
	}
	...
	sealed class SaveModulesCtxMenuCommand : ModulesCtxMenuCommand {
		...
		public override string? GetHeader(ModulesCtxMenuContext context) {
			...
			return count > 1 ?
				string.Format(NetSpy_Debugger_Resources.SaveModulesCommand, count) :
				NetSpy_Debugger_Resources.SaveModuleCommand;
		}
	}
}
