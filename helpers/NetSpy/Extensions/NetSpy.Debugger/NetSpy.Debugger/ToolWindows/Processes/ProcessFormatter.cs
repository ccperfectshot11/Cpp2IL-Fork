...
namespace NetSpy.Debugger.ToolWindows.Processes {
	...
	sealed class ProcessFormatter {
		...
		static string GetStateText(DbgProcessState state) {
			switch (state) {
			case DbgProcessState.Running:	return NetSpy_Debugger_Resources.Process_Running;
			case DbgProcessState.Paused:	return NetSpy_Debugger_Resources.Process_Paused;
