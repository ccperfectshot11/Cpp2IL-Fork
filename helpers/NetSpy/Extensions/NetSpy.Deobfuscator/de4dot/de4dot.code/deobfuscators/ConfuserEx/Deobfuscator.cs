...
namespace de4dot.code.deobfuscators.ConfuserEx
{
	public class DeobfuscatorInfo : DeobfuscatorInfoBase
	{
		...
		private class Deobfuscator : DeobfuscatorBase
		{
			...
			private AntiTamperRemover _antiTamperRemover;
			...
			protected override void ScanForObfuscator()
			{
				...
				_antiTamperRemover = new AntiTamperRemover(module);
				_antiTamperRemover.Find();
			}
			...
			public override void DeobfuscateEnd()
			{
				...
				// ConfuserEx AntiTamper leaves behind <Module> methods whose IL bodies are
				// unparseable garbage (e.g. the integrity-check thread body). Neutralize
				// those bodies so they can't reach the metadata writer as garbage, and
				// remove the ones that end up unreferenced.
				if (_antiTamperRemover != null && _antiTamperRemover.Detected) {
					Logger.n("ConfuserEx AntiTamper: neutralizing {0} corrupt <Module> method(s)",
						System.Linq.Enumerable.Count(_antiTamperRemover.Methods));
					var removable = _antiTamperRemover.Fix();
					foreach (var method in removable)
						AddMethodToBeRemoved(method, "ConfuserEx AntiTamper method");
				}

				// NetSpy extra cleanup: rewrite ConfuserEx reference-proxy thin wrappers to their
				// real target call, then strip the anti-debug / anti-dump helper methods that
				// ConfuserEx injects into <Module>. de4dot's built-in passes miss these on modern
				// builds, leaving smethod_X calls scattered through the decompiled output.
				var proxyMethods = ConfuserExCleanup.FixProxyCalls(module);
				if (proxyMethods.Count > 0) {
					Logger.n("ConfuserEx: inlined {0} reference-proxy call(s)", proxyMethods.Count);
					AddMethodsToBeRemoved(proxyMethods, "ConfuserEx reference-proxy method");
				}
				var antiMethods = ConfuserExCleanup.RemoveAntiDebugDump(module);
				if (antiMethods.Count > 0) {
					Logger.n("ConfuserEx: removed {0} anti-debug/anti-dump method(s)", antiMethods.Count);
					AddMethodsToBeRemoved(antiMethods, "ConfuserEx anti-debug/anti-dump method");
				}
