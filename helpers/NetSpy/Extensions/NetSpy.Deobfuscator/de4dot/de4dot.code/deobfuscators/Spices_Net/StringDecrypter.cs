...
namespace de4dot.code.deobfuscators.Spices_Net {
	class StringDecrypter {
		...
		// NetSpy: newer Spices.Net (v5) emits a per-build-randomized string-data transform method.
		// When detected (isSpices5) it is executed by Spices5Emulator instead of the flags path.
		MethodDef transformMethod;
		bool isSpices5;
		...
		// NetSpy: Spices.Net v5 randomizes the string-data transform per build, but always emits it
		// (a pure byte[]->byte[] method) into the assembly so it can self-decrypt at runtime. It is
		// recognized by its use of the assembly public key. We reproduce it by executing that method
		// with a tiny IL interpreter (Spices5Emulator) rather than matching any fixed formula.
		void AnalyzeSpices5() {
			if (transformMethod == null || transformMethod.Body == null)
				return;
			foreach (var ins in transformMethod.Body.Instructions) {
				if ((ins.OpCode.Code == Code.Call || ins.OpCode.Code == Code.Callvirt) &&
					ins.Operand is IMethod m && (m.Name == "GetPublicKey" || m.Name == "GetPublicKeyToken")) {
					isSpices5 = true;
					return;
				}
			}
		...
		void InitializeSpices5() {
			decryptedData = new Spices5Emulator(module).Run(transformMethod, (byte[])encryptedDataField.InitialValue.Clone());
		}
		...
		bool CheckCctor(MethodDef cctor, out FieldDef compressedDataField, out StringDataFlags flags) {
			...
			for (int i = 0; i < instructions.Count; i++) {
				...
				if (index < instructions.Count && instructions[index].OpCode.Code == Code.Call) {
					transformMethod = instructions[index].Operand as MethodDef;
					flags = GetStringDataFlags(transformMethod);
				}
