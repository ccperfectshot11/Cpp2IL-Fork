...
namespace NetSpy.Scripting.Roslyn.VisualBasic {
	sealed class VisualBasicControlVM : ScriptControlVM {
		...
		static readonly string CODEFILTERTEXT = $"{NetSpy_Scripting_Roslyn_Resources.VisualBasicScriptFiles} (*.vbx)|*.vbx|{NetSpy_Scripting_Roslyn_Resources.AllFiles} (*.*)|*.*";

		protected override string Help => NetSpy_Scripting_Roslyn_Resources.HelpString;
		...
		protected override void InitializeUserScriptOptions(UserScriptOptions options) {
			...
			if (rspFile is null)
				return;
			ReplEditor.OutputPrintLine(string.Format(NetSpy_Scripting_Roslyn_Resources.LoadingContextFromFile, Path.GetFileName(rspFile)), BoxedTextColor.ReplOutputText);
