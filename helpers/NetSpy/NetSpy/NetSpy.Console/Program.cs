...
using System.Text;
using dnlib.DotNet;
using NetSpy.Console.Properties;
...
namespace NetSpy_Console {
	...
	static class Program {
		static int Main(string[] args) {
			...
			try {
				...
				return new NetSpyDecompiler().Run(args);
			}
			catch (Exception ex) {
	...
	sealed class NetSpyDecompiler : IMSBuildProjectWriterLogger {
...
#if NET
		readonly NetSpy.MainApp.DotNetAssemblyLoader dotNetAssemblyLoader = new NetSpy.MainApp.DotNetAssemblyLoader(System.Runtime.Loader.AssemblyLoadContext.Default);
#endif

		static readonly char PATHS_SEP = Path.PathSeparator;

		public NetSpyDecompiler() {
#if NET
			...
			moduleContext = ModuleDef.CreateModuleContext(); // Same as NetSpy.exe
			assemblyResolver = (AssemblyResolver)moduleContext.AssemblyResolver;
			assemblyResolver.EnableFrameworkRedirect = false; // Same as NetSpy.exe
			assemblyResolver.FindExactMatch = true; // Same as NetSpy.exe
		...
		static IBamlDecompiler? TryLoadBamlDecompiler() => TryCreateType<IBamlDecompiler>("NetSpy.BamlDecompiler.x", "NetSpy.BamlDecompiler.BamlDecompiler");
		...
		public int Run(string[] args) {
			try {
				...
				if (allLanguages.Length == 0)
					throw new ErrorException(NetSpy_Console_Resources.NoLanguagesFound);
				if (GetLanguageOrNull() is null)
					throw new ErrorException(string.Format(NetSpy_Console_Resources.LanguageXDoesNotExist, language));
				Decompile();
			}
			catch (ErrorException ex) {
				PrintHelp();
				Console.WriteLine();
				Console.WriteLine(NetSpy_Console_Resources.Error1, ex.Message);
				return 1;
			}
			catch (Exception ex) {
				Dump(ex);
		...
		void PrintHelp() {
			...
			Console.WriteLine(progName + " " + NetSpy_Console_Resources.UsageHeader, progName);
			Console.WriteLine();
			...
			Console.WriteLine();
			Console.WriteLine(NetSpy_Console_Resources.Languages);
			...
			if (langLists.Length > 0) {
				Console.WriteLine();
				Console.WriteLine(NetSpy_Console_Resources.LanguageOptions);
				Console.WriteLine(NetSpy_Console_Resources.LanguageOptionsDesc);
			...
			Console.WriteLine();
			Console.WriteLine(NetSpy_Console_Resources.ExamplesHeader);
		...
		static readonly UsageInfo[] usageInfos = new UsageInfo[] {
			new UsageInfo("--sdk-project", null, NetSpy_Console_Resources.CmdLineDescription_SdkProject),
			new UsageInfo("--asm-path", NetSpy_Console_Resources.CmdLinePath, NetSpy_Console_Resources.CmdLineDescription_AsmPath),
			new UsageInfo("--user-gac", NetSpy_Console_Resources.CmdLinePath, NetSpy_Console_Resources.CmdLineDescription_UserGAC),
			new UsageInfo("--no-gac", null, NetSpy_Console_Resources.CmdLineDescription_NoGAC),
			new UsageInfo("--no-stdlib", null, NetSpy_Console_Resources.CmdLineDescription_NoStdLib),
			new UsageInfo("--no-sln", null, NetSpy_Console_Resources.CmdLineDescription_NoSLN),
			new UsageInfo("--sln-name", NetSpy_Console_Resources.CmdLineName, NetSpy_Console_Resources.CmdLineDescription_SlnName),
			new UsageInfo("--threads", "N", NetSpy_Console_Resources.CmdLineDescription_NumberOfThreads),
			new UsageInfo("--no-resources", null, NetSpy_Console_Resources.CmdLineDescription_NoResources),
			new UsageInfo("--no-resx", null, NetSpy_Console_Resources.CmdLineDescription_NoResX),
			new UsageInfo("--no-baml", null, NetSpy_Console_Resources.CmdLineDescription_NoBAML),
			new UsageInfo("--no-color", null, NetSpy_Console_Resources.CmdLineDescription_NoColor),
			new UsageInfo("--spaces", "N", NetSpy_Console_Resources.CmdLineDescription_Spaces),
			new UsageInfo("--vs", "N", string.Format(NetSpy_Console_Resources.CmdLineDescription_VSVersion, 2017)),
			new UsageInfo("--project-guid", "N", NetSpy_Console_Resources.CmdLineDescription_ProjectGUID),
			new UsageInfo("-t", NetSpy_Console_Resources.CmdLineName, NetSpy_Console_Resources.CmdLineDescription_Type1),
			new UsageInfo("--type", NetSpy_Console_Resources.CmdLineName, NetSpy_Console_Resources.CmdLineDescription_Type2),
			new UsageInfo("--md", "N", NetSpy_Console_Resources.CmdLineDescription_MDToken),
			new UsageInfo("--gac-file", NetSpy_Console_Resources.CmdLineAssembly, NetSpy_Console_Resources.CmdLineDescription_GACFile),
			new UsageInfo("-r", null, NetSpy_Console_Resources.CmdLineDescription_RecursiveSearch),
			new UsageInfo("-o", NetSpy_Console_Resources.CmdLineOutputDir, NetSpy_Console_Resources.CmdLineDescription_OutputDirectory),
			new UsageInfo("-l", NetSpy_Console_Resources.CmdLineLanguage, NetSpy_Console_Resources.CmdLineDescription_Language),
		};
		...
		static readonly HelpInfo[] helpInfos = new HelpInfo[] {
			new HelpInfo(NetSpy_Console_Resources.ExampleDescription1, @"-o C:\out\path C:\some\path"),
			new HelpInfo(NetSpy_Console_Resources.ExampleDescription2, @"-o C:\out\path -r C:\some\path"),
			new HelpInfo(NetSpy_Console_Resources.ExampleDescription3, @"-o C:\out\path C:\some\path\*.dll"),
			new HelpInfo(NetSpy_Console_Resources.ExampleDescription4, @"--md 0x06000123 file.dll"),
			new HelpInfo(NetSpy_Console_Resources.ExampleDescription5, @"-t system.int32 --gac-file ""mscorlib, Version=4.0.0.0"""),
		};
		...
		void Dump(Exception? ex) {
			while (ex is not null) {
				Console.WriteLine(NetSpy_Console_Resources.Error1, ex.GetType());
		...
		void ParseCommandLine(string[] args) {
			if (args.Length == 0)
				throw new ErrorException(NetSpy_Console_Resources.MissingOptions);
			...
			for (int i = 0; i < args.Length; i++) {
				...
				if (canParseCommands && arg[0] == '-') {
					...
					case "--output-dir":
						if (next is null)
							throw new ErrorException(NetSpy_Console_Resources.MissingOutputDir);
					...
					case "--lang":
						if (next is null)
							throw new ErrorException(NetSpy_Console_Resources.MissingLanguageName);
						language = next;
						i++;
						if (GetLanguageOrNull() is null)
							throw new ErrorException(string.Format(NetSpy_Console_Resources.LanguageDoesNotExist, language));
						lang = null;
						langDict = null;
						break;
					...
					case "--asm-path":
						if (next is null)
							throw new ErrorException(NetSpy_Console_Resources.MissingAsmSearchPath);
					...
					case "--user-gac":
						if (next is null)
							throw new ErrorException(NetSpy_Console_Resources.MissingUserGacPath);
					...
					case "--sln-name":
						if (next is null)
							throw new ErrorException(NetSpy_Console_Resources.MissingSolutionName);
						slnName = next;
						i++;
						if (Path.IsPathRooted(slnName))
							throw new ErrorException(string.Format(NetSpy_Console_Resources.InvalidSolutionName, slnName));
						break;

					case "--threads":
						if (next is null)
							throw new ErrorException(NetSpy_Console_Resources.MissingNumberOfThreads);
						i++;
					...
					case "--vs":
						if (next is null)
							throw new ErrorException(NetSpy_Console_Resources.MissingVSVersion);
						i++;
						int vsVer;
						...
						default: throw new ErrorException(string.Format(NetSpy_Console_Resources.InvalidVSVersion, vsVer));
						}
						break;
					...
					case "--spaces":
						if (next is null)
							throw new ErrorException(NetSpy_Console_Resources.MissingArgument);
						const int MIN_SPACES = 0, MAX_SPACES = 100;
						if (!int.TryParse(next, out spaces) || spaces < MIN_SPACES || spaces > MAX_SPACES)
							throw new ErrorException(string.Format(NetSpy_Console_Resources.InvalidSpacesArgument, MIN_SPACES, MAX_SPACES));
						i++;
						break;
					...
					case "--type":
						if (next is null)
							throw new ErrorException(NetSpy_Console_Resources.MissingTypeName);
						i++;
						typeName = next;
						break;
					...
					case "--md":
						if (next is null)
							throw new ErrorException(NetSpy_Console_Resources.MissingMDToken);
						i++;
					...
					case "--gac-file":
						if (next is null)
							throw new ErrorException(NetSpy_Console_Resources.MissingGacFile);
						i++;
						gacFiles.Add(next);
						break;
					...
					case "--project-guid":
						if (next is null || !Guid.TryParse(next, out projectGuid))
							throw new ErrorException(NetSpy_Console_Resources.InvalidGuid);
						i++;
						break;
					...
					default:
						...
						if (langDict.TryGetValue(arg, out tuple)) {
							...
							if (hasArg && next is null)
								throw new ErrorException(NetSpy_Console_Resources.MissingOptionArgument);
							if (hasArg)
								i++;
						...
						throw new ErrorException(string.Format(NetSpy_Console_Resources.InvalidOption, arg));
					}
				}
				else
					files.Add(arg);
		...
		void Decompile() {
			...
			if (mdToken != 0 || typeName is not null) {
				if (files.Count == 0)
					throw new ErrorException(NetSpy_Console_Resources.MissingDotNetFilename);
				if (files.Count != 1)
					throw new ErrorException(NetSpy_Console_Resources.OnlyOneFileCanBeDecompiled);
				...
				if (member is null) {
					if (typeName is not null)
						throw new ErrorException(string.Format(NetSpy_Console_Resources.CouldNotFindTypeX, typeName));
					throw new ErrorException(NetSpy_Console_Resources.InvalidToken);
				}
				...
				if (member is MethodDef)
					...
					throw new ErrorException(NetSpy_Console_Resources.InvalidMemberToDecompile);
			}
			else {
				if (string2.IsNullOrEmpty(outputDir))
					throw new ErrorException(NetSpy_Console_Resources.MissingOutputDir);
				if (GetLanguage().ProjectFileExtension is null)
					throw new ErrorException(string.Format(NetSpy_Console_Resources.LanguageXDoesNotSupportProjects, GetLanguage().UniqueNameUI));
		...
		IEnumerable<ProjectModuleOptions> GetDotNetFiles() {
			foreach (var file in files) {
				if (File.Exists(file)) {
					...
					if (info is null)
						throw new Exception(string.Format(NetSpy_Console_Resources.NotDotNetFile, file));
					yield return info;
				}
					...
					if (Directory.Exists(path)) {
						...
						throw new ErrorException(string.Format(NetSpy_Console_Resources.FileOrDirDoesNotExist, file));
				}
			}
			...
			foreach (var asmName in gacFiles) {
				...
				if (asm is null)
					throw new ErrorException(string.Format(NetSpy_Console_Resources.CouldNotResolveGacFileX, asmName));
		...
		public void Error(string message) {
			errors++;
			Console.Error.WriteLine(string.Format(NetSpy_Console_Resources.Error1, message));
		}
		int errors;
