...
namespace NetSpy.Contracts.App {
	...
	public interface IDsLoader {
		/// <summary>
		/// Called when NetSpy exits
		/// </summary>
		/// <param name="settingsService">Settings manager</param>
		void Save(ISettingsService settingsService);

		/// <summary>
		/// Called when NetSpy has just started. If the method takes too long to execute, give control
		/// back to NetSpy by using yield return. Only values in <see cref="LoaderConstants"/>
