...
namespace NetSpy.Contracts.Documents.TreeView.Resources {
	...
	public static class SaveResources {
		...
		public static void Save(IResourceDataProvider[]? nodes, bool useSubDirs, ResourceDataType resourceDataType, Window? ownerWindow = null) {
			...
			win.Title = files.Length == 1 ? NetSpy_Contracts_NetSpy_Resources.SaveResource : NetSpy_Contracts_NetSpy_Resources.SaveResources;
			var res = win.ShowDialog();
			if (res != true)
				return;
			if (!data.WasError)
				return;
			MsgBox.Instance.Show(string.Format(NetSpy_Contracts_NetSpy_Resources.AnErrorOccurred, data.ErrorMessage));
		}
