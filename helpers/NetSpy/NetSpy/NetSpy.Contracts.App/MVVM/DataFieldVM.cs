...
namespace NetSpy.Contracts.MVVM {
		...
		protected override string? Validate() {
			try {
				...
				if (!string.IsNullOrEmpty(ex.Message))
					return ex.Message;
				return string.Format(NetSpy_Contracts_NetSpy_Resources.CouldNotConvert, StringValue);
			}
		}
	}
	...
	public class NullableGuidVM : DataFieldVM<Guid?> {
		...
		internal static Guid ParseGuid(string s, out string? error) {
			...
			error = NetSpy_Contracts_NetSpy_Resources.InvalidGuid;
			return Guid.Empty;
		}
	}
	...
	public class DefaultConverterVM<T> : DataFieldVM<T> {
		...
		protected override string? ConvertToValue(out T value) {
			...
			try {
				...
				value = default!;
				error = string.Format(NetSpy_Contracts_NetSpy_Resources.ValueMustBeType, typeof(T).FullName, ex.Message);
			}
			return error;
		}
	}
