...
namespace NetSpy.Contracts.ToolWindows.Search {
	sealed class SearchMatcher {
		...
		public string GetHelpText() {
			...
			foreach (var def in definitions) {
				...
				sb.Append(" <text> ");
				sb.Append(string.Format(NetSpy_Contracts_NetSpy_Resources.Search_SearchColumnHelpText, def.LocalizedName));
				sb.AppendLine();
			}
			...
			if (definitions.Length > 0) {
				...
				sb.AppendLine(string.Format(NetSpy_Contracts_NetSpy_Resources.Search_SearchColumnInvertMatchHelpText, "-option!", "-" + def.ShortOptionName + "! <text>"));
			}
			return sb.ToString();
		}
