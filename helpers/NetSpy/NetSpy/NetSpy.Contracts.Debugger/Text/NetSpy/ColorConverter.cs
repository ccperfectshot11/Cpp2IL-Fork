...
#pragma warning disable CS1591 // Missing XML comment for publicly visible type or member
	public static class ColorConverter {
		...
		static readonly Dictionary<DbgTextColor, object> toNetSpyColor;

		static ColorConverter() {
			toDebuggerColor = new Dictionary<object, DbgTextColor>();
			toNetSpyColor = new Dictionary<DbgTextColor, object>();
		...
		static void AddColors(short[] colors) {
			for (int i = 0; i < colors.Length; i += 2) {
				...
				var NetSpyColor = ((TextColor)colors[i + 1]).Box();
				toDebuggerColor.Add(NetSpyColor, debuggerColor);
				toNetSpyColor.Add(debuggerColor, NetSpyColor);
			}
		}
		...
		public static object ToNetSpyColor(DbgTextColor color) {
			if (!toNetSpyColor.TryGetValue(color, out var netspyColor)) {
				Debug.Fail($"Couldn't convert color '{color}'");
				netspyColor = TextColor.Error.Box();
			}
			return netspyColor;
		}
	}
