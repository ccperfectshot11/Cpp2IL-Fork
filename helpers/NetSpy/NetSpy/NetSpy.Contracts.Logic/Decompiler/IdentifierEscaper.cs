...
namespace NetSpy.Contracts.Decompiler {
	...
	public static class IdentifierEscaper {
		...
		// NetSpy: must be a valid C# identifier (no '<'/'>') so empty-named members produced by
		// obfuscators don't render as "<<EMPTY_NAME>>" and break the build.
		const string EMPTY_NAME = "__EMPTY_NAME__";
		...
		public static string Escape(string? id, int maxLength, bool allowSpaces) {
			...
			for (; i < id.Length; i++) {
				...
				if (!IsValidChar(c, allowSpaces)) {
					// NetSpy: '<' and '>' appear in compiler-generated names (eg. the
