...
using System.Text;
using NetSpy.Contracts.Logic.Properties;
...
namespace NetSpy.Contracts.Utilities {
	...
	public static class SimpleTypeConverter {
		...
		public static byte[]? ParseByteArray(string s, out string? error) {
			...
			s = s.Replace("\u2029", string.Empty);
			if (s.Length % 2 != 0) {
				error = NetSpy_Contracts_Logic_Resources.InvalidHexStringSize;
				return null;
			}
			...
			for (int i = 0; i < s.Length; i += 2) {
				...
				if (upper < 0 || lower < 0) {
					error = NetSpy_Contracts_Logic_Resources.InvalidHexCharacter;
					return null;
				}
		...
		static string? TryParseUnsigned(string s, ulong min, ulong max, out ulong value) {
			...
			if (!isValid) {
				if (s.StartsWith("-", StringComparison.OrdinalIgnoreCase))
					return NetSpy_Contracts_Logic_Resources.InvalidUnsignedInteger1;
				return NetSpy_Contracts_Logic_Resources.InvalidUnsignedInteger2;
			}
			if (value < min || value > max) {
				if (min == 0)
					return string.Format(NetSpy_Contracts_Logic_Resources.InvalidUnsignedInteger3, min, max);
				return string.Format(NetSpy_Contracts_Logic_Resources.InvalidUnsignedInteger4, min, max);
			}
		...
		public static float ParseSingle(string s, out string? error) {
			...
			error = NetSpy_Contracts_Logic_Resources.InvalidSingle;
			return 0;
		}
		...
		public static double ParseDouble(string s, out string? error) {
			...
			error = NetSpy_Contracts_Logic_Resources.InvalidDouble;
			return 0;
		}
		...
		public static decimal ParseDecimal(string s, out string? error) {
			...
			error = NetSpy_Contracts_Logic_Resources.InvalidDecimal;
			return 0;
		}
		...
		public static DateTime ParseDateTime(string s, out string? error) {
			...
			error = NetSpy_Contracts_Logic_Resources.InvalidDateTime;
		...
		public static TimeSpan ParseTimeSpan(string s, out string? error) {
			...
			error = NetSpy_Contracts_Logic_Resources.InvalidTimeSpan;
		...
		public static bool ParseBoolean(string s, out string? error) {
			...
			error = NetSpy_Contracts_Logic_Resources.InvalidBoolean;
			return false;
		}
		...
		static char SetParseCharError(out string? error) {
			error = NetSpy_Contracts_Logic_Resources.InvalidChar;
			return (char)0;
		}
		...
		static char ParseChar(string s, ref int index, out string? error) {
			...
			if (c == '\\') {
				...
				default:
					error = string.Format(NetSpy_Contracts_Logic_Resources.InvalidEscapeSequence, c);
					return (char)0;
				}
			}
			if (index >= s.Length)
		...
		static string? SetParseStringError(bool canHaveNull, out string? error) {
			error = canHaveNull ?
				NetSpy_Contracts_Logic_Resources.InvalidString1 :
				NetSpy_Contracts_Logic_Resources.InvalidString2;
			return null;
		}
		...
		static string? ParseString(string s, bool canHaveNull, ref int index, out string? error) {
			...
			while (true) {
				...
				if (c == '\\') {
					...
					default:
						error = string.Format(NetSpy_Contracts_Logic_Resources.InvalidEscapeSequence2, c);
						return null;
					}
				}
				else
		...
		static string? TryParseSigned(string s, long min, long max, object minObject, out long value) {
			...
			if (!isValid)
				return NetSpy_Contracts_Logic_Resources.InvalidInteger1;
			if (isSigned) {
				if (value2 > (ulong)long.MaxValue + 1)
					return NetSpy_Contracts_Logic_Resources.InvalidInteger2;
				value = unchecked(-(long)value2);
			}
			else {
				if (value2 > (ulong)long.MaxValue)
					return NetSpy_Contracts_Logic_Resources.InvalidInteger3;
				value = (long)value2;
			}
			if (value < min || value > max) {
				if (min == 0)
					return string.Format(NetSpy_Contracts_Logic_Resources.InvalidInteger4, min, max);
				return string.Format(NetSpy_Contracts_Logic_Resources.InvalidInteger5, minObject, max, min < 0 ? "-" : string.Empty);
			}
		...
		static T[]? ParseList<T>(string s, out string? error, Func<string, (T value, string? error)> parseValue) {
			...
			foreach (var elem in s.Split(',')) {
				...
				if (value == string.Empty) {
					error = NetSpy_Contracts_Logic_Resources.InvalidListValue;
					return null;
				}
		...
		static T[]? ParseList<T, U>(string s, out string? error, ParseListCallBack<T, U> parseValue, U data) {
			...
			while (true) {
				...
				if (index >= s.Length)
					break;
				if (s[index] != ',') {
					error = NetSpy_Contracts_Logic_Resources.InvalidListValue2;
					return null;
				}
				index++;
			}
