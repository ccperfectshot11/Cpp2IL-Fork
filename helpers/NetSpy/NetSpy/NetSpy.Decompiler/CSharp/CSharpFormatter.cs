...
namespace NetSpy.Decompiler.CSharp {
	public struct CSharpFormatter {
		...
		void WriteDeprecated(bool isDeprecated) {
			if (isDeprecated) {
				...
				OutputWrite(NetSpy_Decompiler_Resources.CSharp_Deprecated_Member, BoxedTextColor.Text);
		...
		void Write(MemberSpecialFlags flags) {
			...
			if ((flags & MemberSpecialFlags.Awaitable) != 0) {
				comma = true;
				OutputWrite(NetSpy_Decompiler_Resources.CSharp_Awaitable_Method, BoxedTextColor.Text);
			}
			if ((flags & MemberSpecialFlags.Extension) != 0) {
				if (comma)
					WriteCommaSpace();
				OutputWrite(NetSpy_Decompiler_Resources.CSharp_Extension_Method, BoxedTextColor.Text);
			}
