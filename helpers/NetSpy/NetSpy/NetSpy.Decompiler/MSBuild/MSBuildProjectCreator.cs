...
namespace NetSpy.Decompiler.MSBuild {
			...
			try {
				...
				Parallel.ForEach(jobs, opts, job => {
					...
					try {
						...
						if (job is IFileJob fjob)
							logger.Error(string.Format(NetSpy_Decompiler_Resources.MSBuild_FileCreationFailed3, fjob.Filename, job.Description, ex.Message));
						else
							logger.Error(string.Format(NetSpy_Decompiler_Resources.MSBuild_FileCreationFailed2, job.Description, ex.Message));
					}
				...
				Parallel.ForEach(projects, opts, p => {
					...
					try {
						...
						logger.Error(string.Format(NetSpy_Decompiler_Resources.MSBuild_FailedToCreateProjectFile, p.Filename, ex.Message));
					}
					progressListener.SetProgress(Interlocked.Increment(ref totalProgress));
				});
				if (writeSolutionFile) {
					...
					try {
						...
						logger.Error(string.Format(NetSpy_Decompiler_Resources.MSBuild_FailedToCreateSolutionFile, SolutionFilename, ex.Message));
					}
					progressListener.SetProgress(Interlocked.Increment(ref totalProgress));
				}
