...
namespace NetSpy.Decompiler.MSBuild {
	sealed class Project {
		...
		public void CreateProjectFiles(DecompileContext ctx) {
			...
			// NetSpy: always allow unsafe. The UnverifiableCodeAttribute check misses unsafe
			// code in many real assemblies (esp. deobfuscated / Il2Cpp interop), causing CS0227
			// on export. Enabling it is harmless for assemblies that don't use unsafe.
			...
			foreach (var dir in dirs) {
				...
				try {
					...
					if (errors++ < 20)
						ctx.Logger.Error(string.Format(NetSpy_Decompiler_Resources.MSBuild_CouldNotCreateDirectory2, dir, ex.Message));
				}
			}
		}
