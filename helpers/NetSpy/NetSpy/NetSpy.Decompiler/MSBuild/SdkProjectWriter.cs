	...
	sealed class SdkProjectWriter : ProjectWriterBase {
		...
		public override void Write() {
			...
			using (var writer = XmlWriter.Create(project.Filename, settings)) {
				...
				// NetSpy: the decompiler can emit modern C# (eg. the 'field' keyword). On net48
				// the default is C# 7.3, which rejects it (CS8370). Use the latest language so the
				// exported code compiles regardless of target framework.
