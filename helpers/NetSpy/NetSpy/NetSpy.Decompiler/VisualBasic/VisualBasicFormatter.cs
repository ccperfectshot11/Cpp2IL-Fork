...
namespace NetSpy.Decompiler.VisualBasic {
	public struct VisualBasicFormatter {
		...
		void WriteDeprecated(bool isDeprecated) {
			if (isDeprecated) {
				...
				OutputWrite(NetSpy_Decompiler_Resources.VisualBasic_Deprecated_Member, BoxedTextColor.Text);
		...
		void Write(MemberSpecialFlags flags) {
			...
			if ((flags & MemberSpecialFlags.Awaitable) != 0) {
				comma = true;
				OutputWrite(NetSpy_Decompiler_Resources.VisualBasic_Awaitable_Method, BoxedTextColor.Text);
			}
			if ((flags & MemberSpecialFlags.Extension) != 0) {
				if (comma)
					WriteCommaSpace();
				OutputWrite(NetSpy_Decompiler_Resources.VisualBasic_Extension_Method, BoxedTextColor.Text);
			}
		...
		void Write(IField? field, bool isToolTip) {
			...
			if (!isEnumOwner || (fd is not null && !fd.IsLiteral)) {
				if (isToolTip) {
					...
					OutputWrite(isConstant ? NetSpy_Decompiler_Resources.ToolTip_Constant : NetSpy_Decompiler_Resources.ToolTip_Field, BoxedTextColor.Text);
		...
		void WriteToolTip(GenericParam? gp) {
			...
			Write(gp);
			WriteSpace();
			OutputWrite(NetSpy_Decompiler_Resources.ToolTip_GenericParameterInTypeOrMethod, BoxedTextColor.Text);
			WriteSpace();
		...
		public void WriteToolTip(ISourceVariable? variable) {
			...
			OutputWrite(DescriptionParenOpen, BoxedTextColor.Punctuation);
			OutputWrite(isLocal ? NetSpy_Decompiler_Resources.ToolTip_Local : NetSpy_Decompiler_Resources.ToolTip_Parameter, BoxedTextColor.Text);
			OutputWrite(DescriptionParenClose, BoxedTextColor.Punctuation);
			WriteSpace();
