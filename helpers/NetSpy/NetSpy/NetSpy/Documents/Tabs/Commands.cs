...
namespace NetSpy.Documents.Tabs {
	...
	sealed class ToolbarFileOpenCommand : ToolBarButtonCommand {
		...
		public override string? GetToolTip(IToolBarItemContext context) => ToolTipHelper.AddKeyboardShortcut(NetSpy_Resources.OpenToolBarToolTip, NetSpy_Resources.ShortCutKeyCtrlO);
	}
	...
	sealed class OpenListCommand : MenuItemBase {
		...
		public override void Execute(IMenuItemContext context) {
			...
			var vm = new OpenDocumentListVM(syntaxHighlight, classificationFormatMap, textElementProvider, documentListService, labelMsg => messageBoxService.Ask<string>(labelMsg, ownerWindow: win, verifier: s => string.IsNullOrEmpty(s) ? NetSpy_Resources.OpenList_MissingName : string.Empty));
			win.DataContext = vm;
