...
namespace NetSpy.Documents.Tabs {
	...
	static class GoToTokenCommand {
		...
		internal static void ExecuteInternal(IDocumentTabService documentTabService) {
			...
			var member = AskForDef(NetSpy_Resources.GoToToken_Title, resolver);
			if (member is null)
				return;
		...
		static object? AskForDef(string title, ITokenResolver resolver) => MsgBox.Instance.Ask(NetSpy_Resources.GoToToken_Label, null, title, s => {
			...
			if (memberRef is null)
				return string.Format(NetSpy_Resources.GoToToken_InvalidToken, token);
			else if (member is null)
				return string.Format(NetSpy_Resources.GoToToken_CouldNotResolve, token);
			return string.Empty;
		});
