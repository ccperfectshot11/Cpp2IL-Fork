...
#pragma warning restore CS8618 // Non-nullable field is uninitialized.
			...
			public void Execute(ExportToProjectVM vm) {
				...
				NetSpyEventSource.Log.ExportToProjectStart();
				...
				.ContinueWith(t => {
					NetSpyEventSource.Log.ExportToProjectStop();
					var ex = t.Exception;
					if (ex is not null)
						Error(string.Format(NetSpy_Resources.ErrorExceptionOccurred, ex));
					EmtpyErrorList();
					vm.OnExportComplete();
					if (!vm.ExportErrors) {
						dlg.Close();
		...
		static string CreateFilesToExportMessage(ModuleDef[] modules) {
			if (modules.Length == 1)
				return NetSpy_Resources.ExportToProject_ExportFileMessage;
			return string.Format(NetSpy_Resources.ExportToProject_ExportNFilesMessage, modules.Length);
		}
