...
using NetSpy.Contracts.MVVM.Dialogs;
...
namespace NetSpy.Hex.Commands {
	...
	sealed class HexCommandOperationsImpl : HexCommandOperations {
		...
		public override void SaveSelection() {
			...
			win.Title = string.Format(NetSpy_Resources.HexEditorSaveSelection_Title, info.Anchor.ToUInt64(), info.Active.ToUInt64());
			var res = win.ShowDialog();
			if (res != true)
				return;
			if (!data.WasError)
				return;
			messageBoxService.Show(string.Format(NetSpy_Resources.AnErrorOccurred, data.ErrorMessage));
		}
		...
		public override void FillSelection() {
			...
			var res = messageBoxService.Ask<byte?>(NetSpy_Resources.FillSelection_Label, "0xFF", NetSpy_Resources.FillSelection_Title, s => {
