...
namespace NetSpy.Hex.Settings {
	abstract class AdvancedAppSettingsPageBase : AppSettingsPage {
		public sealed override string Title => NetSpy_Resources.AdvancedSettings;
		...
		static readonly EnumVM[] hexColumnLineKindList = new EnumVM[6] {
			new EnumVM(HexColumnLineKind.None, NetSpy_Resources.BlockStructureLineKind_None),
			new EnumVM(HexColumnLineKind.Solid, NetSpy_Resources.BlockStructureLineKind_SolidLines),
		...
		static string GetDashedText(int px) => NetSpy_Resources.BlockStructureLineKind_DashedLines + " (" + px.ToString() + "px)";
