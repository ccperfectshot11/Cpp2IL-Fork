...
namespace NetSpy.MainApp {
	...
	sealed class AboutScreenDocumentTabContentFactory : IDocumentTabContentFactory {
		...
		AboutScreenDocumentTabContentFactory(IDocumentViewerContentFactoryProvider documentViewerContentFactoryProvider, IAppWindow appWindow, IExtensionService extensionService, IUpdateService updateService, IContentTypeRegistryService contentTypeRegistryService) {
			...
			aboutContentType = contentTypeRegistryService.GetContentType(ContentTypes.AboutNetSpy);
		}
	...
	sealed class AboutScreenDocumentTabContent : DocumentTabContent {
		public override string Title => NetSpy_Resources.About_TabTitle;
...
#error Unknown target framework
#endif
			output.WriteLine($"{Constants.NetSpy} {appWindow.AssemblyInformationalVersion} ({frameworkName})", BoxedTextColor.Text);
			output.WriteLine();
			output.WriteLine(NetSpy_Resources.AboutScreen_LicenseInfo, BoxedTextColor.Text);
			output.WriteLine();

			output.AddUIElement(delegate {
				var button = new Button { Content = NetSpy_Resources.AboutScreen_CheckForUpdates };
			...
			output.WriteLine(NetSpy_Resources.AboutScreen_LoadedFiles, BoxedTextColor.Text);
			...
			output.WriteLine();
			WriteResourceFile(output, "NetSpy.LicenseInfo.CREDITS.txt");
		}
		...
		async void OnUpdateButtonClick(object sender, RoutedEventArgs e) {
			...
			button.Content = NetSpy_Resources.AboutScreen_CheckingForUpdates;

			var updateResult = await updateService.CheckForUpdatesAsync();
			if (!updateResult.Success)
				MsgBox.Instance.Show(NetSpy_Resources.AboutScreen_FailedToRetrieveUpdateInfo);
			else if (!updateResult.UpdateAvailable)
				MsgBox.Instance.Show(NetSpy_Resources.AboutScreen_RunningLatestVersion);
			else {
				var result = MsgBox.Instance.Show(string.Format(NetSpy_Resources.AboutScreen_NewUpdateAvailable, updateResult.VersionInfo.Version), MsgBoxButton.Yes | MsgBoxButton.No);
				if (result == MsgBoxButton.Yes)
					AboutHelpers.OpenWebPage(updateResult.VersionInfo.DownloadUrl, MsgBox.Instance);
			}

			button.Content = NetSpy_Resources.AboutScreen_CheckForUpdates;
		...
		List<Info> GetInfos() {
			...
			infos.Add(new Info(GetType().Assembly, CreateNetSpyInfo()));
		...
		static ExtensionInfo CreateNetSpyInfo() => new ExtensionInfo();
	}
}
