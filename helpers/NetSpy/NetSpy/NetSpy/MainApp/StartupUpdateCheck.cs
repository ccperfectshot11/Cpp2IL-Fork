...
namespace NetSpy.MainApp {
	...
	sealed class StartupUpdateCheck : IAutoLoaded {
		...
		async void CheckForUpdate() {
			...
			string message = string.Format(NetSpy_Resources.InfoBar_NewUpdateAvailable, updateInfo.VersionInfo.Version);
			DisplayNotification(message, InfoBarIcon.Information, new[] {
				new InfoBarInteraction(NetSpy_Resources.InfoBar_OpenDownloadPage, ctx => {
					AboutHelpers.OpenWebPage(updateInfo.VersionInfo.DownloadUrl, messageBoxService);
					ctx.CloseElement();
				}),
				new InfoBarInteraction(NetSpy_Resources.InfoBar_IgnoreThisUpdate, ctx => {
