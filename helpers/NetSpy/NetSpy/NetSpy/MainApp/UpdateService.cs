		...
		static async Task<VersionInfo?> TryGetLatestVersionAsync() {
			try {
				...
				using (var client = new HttpClient(new HttpClientHandler { UseProxy = true, UseDefaultCredentials = true })) {
					client.DefaultRequestHeaders.UserAgent.Add(new ProductInfoHeaderValue("NetSpyEx", currentVersion.ToString()));

					result = await client.GetStringAsync("https://api.github.com/repos/NetSpyEx/NetSpy/releases/latest");
				}
