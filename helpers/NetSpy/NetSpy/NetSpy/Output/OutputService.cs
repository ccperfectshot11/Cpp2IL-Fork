...
using NetSpy.Output.Settings;
using NetSpy.Properties;
using NetSpy.Text.Editor;
...
namespace NetSpy.Output {
	...
	sealed class OutputService : ViewModelBase, IOutputServiceInternal {
		...
		public string ClearAllToolTip => ToolTipHelper.AddKeyboardShortcut(NetSpy_Resources.Output_ClearAll_ToolTip, NetSpy_Resources.ShortCutKeyCtrlL);
		public string SaveToolTip => ToolTipHelper.AddKeyboardShortcut(NetSpy_Resources.Output_Save_ToolTip, NetSpy_Resources.ShortCutKeyCtrlS);
		public string WordWrapToolTip => ToolTipHelper.AddKeyboardShortcut(NetSpy_Resources.Output_WordWrap_ToolTip, NetSpy_Resources.ShortCutKeyCtrlECtrlW);
		...
		static readonly string TEXTFILES_FILTER = $"{NetSpy_Resources.TextFiles} (*.txt)|*.txt|{NetSpy_Resources.AllFiles} (*.*)|*.*";

		string GetFilename(OutputBufferVM vm) {
			// Same as VS2015
			var s = vm.Name.Replace(" ", string.Empty);
			return NetSpy_Resources.Window_Output + "-" + s + ".txt";
		}
