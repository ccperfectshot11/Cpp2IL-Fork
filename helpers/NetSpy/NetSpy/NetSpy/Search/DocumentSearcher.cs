...
namespace NetSpy.Search {
	...
	sealed class DocumentSearcher : IDocumentSearcher {
		...
		void SearchNewThread(object? o) {
			try {
				var searchMsg = SearchResult.CreateMessage(filterSearcherOptions.Context, NetSpy_Resources.Searching, BoxedTextColor.Text, true);
			...
			lock (lockObj) {
				if (totalResultsFound++ >= options.MaxResults) {
					if (totalResultsFound == options.MaxResults + 1)
						AddSearchResultNoCheck(SearchResult.CreateMessage(filterSearcherOptions.Context, string.Format(NetSpy_Resources.SearchAbortedMessage, options.MaxResults), BoxedTextColor.Error, true));
