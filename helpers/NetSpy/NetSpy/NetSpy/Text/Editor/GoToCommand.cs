...
namespace NetSpy.Text.Editor {
	...
	sealed class GoToCommandTargetFilter : ICommandTargetFilter {
		...
		bool GetLineColumn(out int chosenLine, out int? chosenColumn) {
			...
			var res = messageBoxService.Ask(NetSpy_Resources.GoToLine_Label, null, NetSpy_Resources.GoToLine_Title, s => {
		...
		string TryGetRowCol(string s, int currentLine, int maxLines, out int? line, out int? column) {
			...
			if (line is null || columnError) {
				if (string.IsNullOrWhiteSpace(s))
					return NetSpy_Resources.GoToLine_EnterLineNum;
				return string.Format(NetSpy_Resources.GoToLine_InvalidLine, s);
			}
			return string.Empty;
		}
