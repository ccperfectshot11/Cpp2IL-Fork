...
namespace NetSpy.Roslyn.Debugger.ValueNodes {
	sealed class ResultsViewMembersValueNodeProvider : MembersValueNodeProvider {
		...
		static readonly DbgDotNetText valueText = new DbgDotNetText(new DbgDotNetTextPart(DbgTextColor.Text, NetSpy_Roslyn_Resources.DebuggerVarsWindow_ExpandResultsViewMessage));
		static readonly DbgDotNetText resultsViewName = new DbgDotNetText(new DbgDotNetTextPart(DbgTextColor.Text, NetSpy_Roslyn_Resources.DebuggerVarsWindow_ResultsView));
		...
		string? InitializeEnumerableDebugView(DbgEvaluationInfo evalInfo) {
			...
			if (proxyCtor is null) {
				...
				if (proxyCtor is null) {
					...
					if (asm is null)
						return string.Format(NetSpy_Roslyn_Resources.SystemCoreDllNotLoaded, asmFilename);
					return string.Format(NetSpy_Roslyn_Resources.TypeDoesNotExistInAssembly, EnumerableDebugViewHelper.GetDebugViewTypeDisplayName(enumerableType), asmFilename);
				}
			}
